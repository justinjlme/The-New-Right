<?php
/** 
 *  @package WPeMatico Publish 2 Email
**/
if ( !defined('ABSPATH') ) {
...
// Metabox campaigns 
add_action('add_meta_boxes', 'wpemfullcontent_metaboxes', 15, 0);
function wpemfullcontent_metaboxes( ) {  // chequea y agrega campos a campaign y graba en free
	global $pagenow, $post;
	if( !(($pagenow == 'post.php' || $pagenow == 'post-new.php') && $post->post_type=='wpematico' ) ) return false;
	add_meta_box( 'fullcontent-box', __('Full Content Options', 'wpematico' ), 'wpem_fullcontent_box','wpematico','normal', 'default' );
/*	add_action('admin_print_scripts-post.php', 'wpem_fullcontent_scripts');
	add_action('admin_print_scripts-post-new.php', 'wpem_fullcontent_scripts'); 
	add_action('admin_print_styles', 'wpem_fullcontent_styles',10,0);
*/
}

/*function wpem_fullcontent_styles() {
	global $post;
	if($post->post_type != 'wpematico') return $post->ID;
//	wp_enqueue_style('thickbox');
}
			
function wpem_fullcontent_scripts() { // load javascript 
	global $post;
	if($post->post_type != 'wpematico') return $post_id;
	add_action('admin_head', 'wpem_fullcontent_head_js');
}
		
function wpem_fullcontent_head_js() { // load javascript 
...
function wpem_fullcontent_box( $post ) {
	global $post, $campaign_data;
	...
	<p><label><input class="checkbox" onclick="t=jQuery(this);if(t.is(':checked')) t.parent().parent().next().fadeIn(); else t.parent().parent().next().fadeOut();" type="checkbox"<?php checked($campaign_data['campaign_ogimage'],true);?> name="campaign_ogimage" value="1" id="campaign_ogimage"/>
	...
	<p><label><input onclick="t=jQuery(this);if(t.is(':checked')) t.parent().parent().next().fadeIn(); else t.parent().parent().next().fadeOut();" class="checkbox" type="checkbox"<?php checked($campaign_data['campaign_fullauthor'],true);?> name="campaign_fullauthor" value="1" id="campaign_fullauthor"/>
 ...
 *  Save campaign data
 */
add_filter('pro_check_campaigndata', 'wpemfullcontent_check_campaigndata', 15, 2);
function wpemfullcontent_check_campaigndata( $campaign_data = array(), $post_data) {  // chequea y agrega campos a campaign y graba en free
