...
class SPP_Download {
	 ...
	 * @since 1.0.6
	 */
	public static function save_download_id($url, $is_feed_url = false) {
		...
		$download_id = md5($url);
		if( $is_feed_url ) {
			// The user will download the latest episode of a feed.  This changes
			// as often as the feed does, so we want a shorter expiration.
			$settings = get_option( 'spp_player_advanced' );
			$timeout = isset( $settings['cache_timeout'] ) ? $settings['cache_timeout'] : '15';
			if ( $timeout > 60 || $timeout < 5 || !is_numeric( $val ) )
				$timeout = 15;
		} else {
			// In theory, this shouldn't change, so a long expiration is best.
			$timeout = 4 * WEEK_IN_SECONDS;
		}
		set_transient('spp_cacheu_' . $download_id, $url, $timeout);
		return $download_id;
	}
