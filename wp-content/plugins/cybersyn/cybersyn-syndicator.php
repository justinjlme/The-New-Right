...
<div class="wrap">
    ...
    if (isset($_GET["edit-feed-id"])) {
        ...
        $old_url = $csyn_syndicator->feeds[intval($_POST["feed_id"])]['url'];
        if (stripos($new_url, 'http') === 0 && $new_url != $old_url) {
            $query = "UPDATE $wpdb->postmeta SET meta_value = '". $wpdb->prepare($new_url) . "' WHERE meta_key = 'cyberseo_rss_source' AND meta_value = '". $wpdb->prepare($old_url) . "'";
            $wpdb->get_results($query);
            $csyn_syndicator->feeds[intval($_POST["feed_id"])]['url'] = $new_url;
        }
        ...
        $csyn_syndicator->feeds[intval($_POST["feed_id"])]['options']['post_category'] = array_map('absint', @$_POST['post_category']);
        ...
        $csyn_syndicator->feeds[intval($_POST["feed_id"])]['options']['shorten_excerpts'] = abs(intval($_POST['shorten_excerpts']));
