(function( $ ) {
	...
	ShareCounter.getNetworkShareCounts = function( url, providers, callback ) {
		...
		if ( notLoadedProviders.length ) {
			var apiLink = ShareCounter.defaultSettings.shareCountsAPI
				.replace( '{url}', url )
				.replace( '{providers}', notLoadedProviders.join( ',' ) );

			currentRequest.providers = currentRequest.providers.concat( providers );

			currentRequest.xhr = $.get( apiLink, function( data ) {
				$.extend( currentRequest.data, data.shares );
			} );
		}

		currentRequest.xhr.done( function() {
