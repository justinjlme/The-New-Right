...
class smpush_api extends smpush_controller{
  ...
  public function savetoken($printout=true){
    ...
    self::$pushdb->query(self::parse_query("INSERT INTO {tbname} ({token_name},{md5token_name},{type_name},{info_name},{active_name},{latitude_name},{longitude_name},{gpstime_name}) VALUES ('$_REQUEST[device_token]','".md5($_REQUEST['device_token'])."','$device_type','$_REQUEST[device_info]','$_REQUEST[active]','$_REQUEST[latitude]','$_REQUEST[longitude]','".current_time('timestamp')."')"));
  ...
  public function get_link(){
    $this->CheckParams(array('id','platform'));
    global $wpdb;
    ...
    $link = self::cleanString($message['options']['desktop_link']);
    if(empty($link)){
  ...
  public function get_archive(){
    ...
    if(!empty($_REQUEST['deviceID'])){
      ...
      WHERE ".$wpdb->prefix."push_desktop_messages.token='".md5($_REQUEST['deviceID'])."' AND ".$wpdb->prefix."push_desktop_messages.type='$_REQUEST[platform]' ORDER BY ".$wpdb->prefix."push_desktop_messages.timepost ASC";
  ...
  private function saveStats($platid, $action, $msgid=0){
    global $wpdb;
    ...
    if(empty($statid)){
      ...
      $wpdb->query("UPDATE ".$wpdb->prefix."push_statistics SET `stat`=`stat`+1 WHERE id='$statid'");
    }
  }
