...
if ( ! class_exists('PC_hap_infinite_scroll') ) :


final class PC_hap_infinite_scroll {
      ...
      static $option_name_enabled = 'hap_infinite_scroll';
      ...
      static function get_settings() {
            ...
            if ( is_null( self::$settings ) ) {
                  ...
                  $settings = $defaults = array(
                    'type'            => 'scroll',// 'click',// 'scroll', // scroll | click
                  ...
                  $_settings = get_theme_support( 'hap-infinite-scroll' );
                  ...
                  //@hu_addon: Do not force the posts per page
...
*/
                   ...
                   */
                  self::$settings = apply_filters( 'hap_infinite_scroll_settings', $settings );
            }

            /** This filter is documented in modules/infinite-scroll/infinity.php */
             return (object) apply_filters( 'hap_infinite_scroll_settings', self::$settings );
      }
      ...
      function action_template_redirect() {
            ...
            if ( !hu_is_ajax() ) {
              ...
              if ( ! current_theme_supports( 'hap-infinite-scroll' ) || ! self::archive_supports_infinity() )
                  return;
            }
            ...
            wp_register_script(
                'hap-infinite-scroll',
                sprintf('%1$sfront/assets/js/endlessly%2$s.js' , trailingslashit( HAP_INFINITE_BASE_URL ) , ( defined('WP_DEBUG') && true === WP_DEBUG ) ? '' : '.min'),
                array( 'jquery' ),
                '4.0.0',
                true
            );
            ...
            wp_register_style(
                'hap-infinite-scroll',
                sprintf('%1$sfront/assets/css/endlessly%2$s.css' , trailingslashit( HAP_INFINITE_BASE_URL ) , ( defined('WP_DEBUG') && true === WP_DEBUG ) ? '' : '.min'),
                array(),
                '20140422'
            );
            ...
            // Add our scripts.
            wp_enqueue_script( 'hap-infinite-scroll' );

            // Add our default styles.
            wp_enqueue_style( 'hap-infinite-scroll' );
  ...
  function action_wp_head() {
    $this->action_wp_footer_settings();
      ...
      function action_wp_footer_settings() {
            ...
            $click_handle_text = __( 'Older posts', 'hueman' );
            ...
            $js_settings = array(
                  ...
                  //@hu_addon: this is passed to the js that will add it to the infinite query vars
                  //so that we can have this information when doing ajax.
                  //E.g. we use this info to exclude the sticky posts only in home
                  'is_home'         => hu_is_real_home()
            );
  ...
  function action_wp_footer() {
    ...
    ?><script type="text/javascript">
      ...
      function query() {
            ...
            if ( ! isset( $_REQUEST['page'] ) /*|| ! current_theme_supports( 'hap-infinite-scroll' ) */ )
                    die;
            ...
            //@hu_addon:
            //Also exclude the latest retrieved posts to be sure to not have duplication (in subsequent queries).
            //For this purpose in the js, in the succes handler, a list of the ids of the latest retrieved posts will be saved
            //and then added to the $_REQUEST array when performing the ajax request.
            if ( true === isset( $_REQUEST['latest_posts'] ) && is_array( $_REQUEST['latest_posts'] ) ) {
                    $post__not_in = array_unique( array_merge( $post__not_in, array_map( 'intval', (array) $_REQUEST['latest_posts'] ) ) );
            }

            //@hu_addon: exclude stickies only in home
            if ( isset( $_REQUEST[ 'is_home' ] ) && $_REQUEST[ 'is_home' ] ) {
      ...
      function render( $results ) {
            ...
            * - before and after action hooks ( hap__before_infinite_scroll_render_loop, hap__after_infinite_scroll_render_loop)
            * - befpre and after post filtered html ( hap_infinite_post_before, hap_infinite_post_after )
            * - template filter (  'hap_infinite_scroll_template' )
            ...
            */
            ...
            do_action( 'hap__before_infinite_scroll_render_loop' );

            $template       = apply_filters( 'hap_infinite_scroll_template', HU_BASE . 'content.php' );
            ...
            while ( have_posts() ) {
                  ...
                  $results['collection'][ "post-{$_post_id}" ] = $_post_html;

                  /* Used for instance with the grid (not masonry) to render the row wrapper */
                  $before = apply_filters( 'hap_infinite_post_before', '', $template, $results['collection'][ "post-{$_post_id}" ], $results[ 'collection' ] );
                  $after  = apply_filters( 'hap_infinite_post_after', '', $template, $results['collection'][ "post-{$_post_id}" ], $results[ 'collection' ] );

                  echo $before.$_post_html.$after;

            }

            do_action( 'hap__after_infinite_scroll_render_loop' );
      ...
      **/
      ...
      public static function archive_supports_infinity() {
            ...
            $supported = current_theme_supports( 'hap-infinite-scroll' ) && ( is_home() || is_archive() || is_search() );
      ...
      */
      function enqueue_spinner_scripts() {
            if ( ! wp_script_is( 'spin', 'registered' ) ) {
                  wp_register_script(
                      'spin',
                      sprintf('%1$sfront/assets/js/spin%2$s.js' , trailingslashit( HAP_INFINITE_BASE_URL ) , ( defined('WP_DEBUG') && true === WP_DEBUG ) ? '' : '.min'),
                      false,
                      '1.3'
                  );
            }
            ...
            if ( ! wp_script_is( 'jquery.spin', 'registered' ) ) {
                  wp_register_script(
                      'jquery.spin',
                      sprintf('%1$sfront/assets/js/jqueryspin%2$s.js' , trailingslashit( HAP_INFINITE_BASE_URL ) , ( defined('WP_DEBUG') && true === WP_DEBUG ) ? '' : '.min'),
                      array( 'jquery', 'spin' ),
                      '1.3'
                  );
            }
 ...
 */
if ( PC_hap_infinite_scroll::got_infinity() ) {
