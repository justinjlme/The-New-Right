...
class smpush_sendpush extends smpush_controller {
  ...
  private static function getMessagData($msgid) {
    ...
    self::$data['platforms'] = json_decode(self::$data['platforms']);
    self::$data['options'] = unserialize(self::$data['options']);
    if(self::$data['options']['extra_type'] != 'normal'){
      $fields = json_decode(self::$data['options']['extravalue']);
      if(!empty($fields)){
    ...
    if(self::$data['options']['and_extra_type'] != 'normal'){
      $fields = json_decode(self::$data['options']['and_extravalue']);
      if(!empty($fields)){
        foreach($fields as $key => $value){
    ...
    if(self::$data['options']['wp_extra_type'] != 'normal'){
      $fields = json_decode(self::$data['options']['wp_extravalue']);
      if(!empty($fields)){
        foreach($fields as $key => $value){
    ...
    if(self::$data['options']['wp10_extra_type'] != 'normal'){
      $fields = json_decode(self::$data['options']['wp10_extravalue']);
      if(!empty($fields)){
        foreach($fields as $key => $value){
  ...
  private static function archiveMsgLog($message, $sendtime, $sendtype, $options, $desktop_notification = array('chrome','safari','firefox'), $msgid=false) {
    global $wpdb;
  ...
  public static function SendPushMessage($device_token, $device_type, $message, $sendsetting = array(), $sendtime = 0) {
    ...
    self::$sendoptions['query'] = "SELECT {id_name} AS id, {tbname}.{token_name} AS device_token,{tbname}.{type_name} AS device_type FROM {tbname} WHERE {tbname}.{md5token_name}='".md5($device_token)."' AND {tbname}.{type_name}='$device_type'";
  ...
  public static function SendCronPush($ids, $message, $extravalue, $gettype = 'userid', $sendsetting = array(), $sendtime = 0, $channel_filter_ids=false, $gps_loc_filter=false, $template_id=false) {
    ...
    if ($ids == 'all') {
      $sendsetting['query'] = "SELECT {id_name} AS id, {tbname}.{token_name} AS device_token,{tbname}.{type_name} AS device_type $select FROM {tbname} $inner WHERE {tbname}.{active_name}='1' $where $order";
    }
    elseif ($gettype == 'userid') {
      if(is_array($ids)){
        $ids = implode(',', $ids);
      }
      $sendsetting['query'] = "SELECT {id_name} AS id, {tbname}.{token_name} AS device_token,{tbname}.{type_name} AS device_type $select FROM {tbname} $inner WHERE {tbname}.userid IN($ids) AND {tbname}.{active_name}='1' $where $order";
    }
    elseif ($gettype == 'tokenid') {
      $ids = implode(',', $ids);
      $sendsetting['query'] = "SELECT {id_name} AS id, {tbname}.{token_name} AS device_token,{tbname}.{type_name} AS device_type $select FROM {tbname} $inner WHERE {tbname}.{id_name} IN($ids) AND {tbname}.{active_name}='1' $where $order";
    }
    elseif ($gettype == 'channel') {
      $defconid = self::$apisetting['def_connection'];
      $tablename = $wpdb->prefix.'push_relation';
      $sendsetting['query'] = "SELECT {id_name} AS id, {tbname}.{token_name} AS device_token,{tbname}.{type_name} AS device_type $select FROM $tablename
    ...
    $msgid = self::archiveMsgLog($message, $sendtime, 'custom', self::$sendoptions);
    return $msgid;
  }
  ...
  public static function send_notification() {
    ...
    if ($_POST) {
      if (isset($_POST['message'])) {
        ...
        if(empty($_POST['platforms']) || (isset($_POST['platforms']['ios']) && isset($_POST['platforms']['android']) && isset($_POST['platforms']['wp']) && isset($_POST['platforms']['bb'])
           && isset($_POST['platforms']['chrome']) && isset($_POST['platforms']['safari']) && isset($_POST['platforms']['firefox']))){
    ...
    elseif (isset($_GET['lastid'], $_GET['increration'])) {
      ...
      if ($queuecount > 0 && empty($_GET['id'])) {
        ...
        wp_enqueue_script('smpush-jquery-ui');
        ...
        wp_enqueue_script('smpush-emojis');
        add_thickbox();
        if(!empty($_GET['id'])){
  ...
  public static function calculateDevices($msgid=false) {
    ...
    if (!empty($_POST['inchannels_and']) OR ! empty($_POST['inchannels_or']) OR ! empty($_POST['notchannels_and']) OR ! empty($_POST['notchannels_or'])) {
      ...
      $tokenstats = self::$pushdb->get_results(self::parse_query("SELECT {tbname}.{id_name} AS tokenid,{tbname}.{type_name} AS device_type $select FROM {tbname} WHERE {active_name}='1' $gpswhere $order"), 'ARRAY_A');
      if(!empty($tokenstats)){
  ...
  public static function connectPush($message, $device_token, $device_type, $options, $showerror = true, $all_count = 0, $cronjob = false, $msgid=0) {
    ...
    elseif ($device_type == 'android') {
      ...
      if (isset($device_token['id'])) {
        self::updateStats('androidsend', count($device_token['id']), $cronjob, $msgid);
        if ($cronjob === false) {
          $ids = implode(',', $device_token['id']);
          $wpdb->query("DELETE FROM ".$wpdb->prefix."push_queue WHERE id IN($ids)");
        }
        if (self::$sendoptions['feedback'] == 1) {
          $wpdb->insert($wpdb->prefix.'push_feedback', array('tokens' => serialize($device_token['token']), 'feedback' => $result, 'device_type' => 'android', 'msgid' => $msgid));
        }
      }
      curl_close($chandle);
    ...
    elseif ($device_type == 'chrome') {
      ...
      if (isset($device_token['id'])) {
        self::updateStats('chromesend', count($device_token['id']), $cronjob, $msgid);
        if ($cronjob === false) {
          $ids = implode(',', $device_token['id']);
          $wpdb->query("DELETE FROM ".$wpdb->prefix."push_queue WHERE id IN($ids)");
        }
        if (self::$sendoptions['feedback'] == 1) {
          $wpdb->insert($wpdb->prefix.'push_feedback', array('tokens' => serialize($device_token['token']), 'feedback' => $result, 'device_type' => 'chrome', 'msgid' => $msgid));
        }
      }
    ...
    elseif ($device_type == 'firefox') {
      ...
      foreach($device_token['token'] as $token){
        ...
        if (($httpcode == 404 || $httpcode == 410) && self::$sendoptions['feedback'] == 1) {
          $wpdb->insert($wpdb->prefix.'push_feedback', array('tokens' => $token, 'device_type' => 'firefox', 'msgid' => $msgid));
        }
  ...
  public static function connectFeedback($all_count, $cronjob = false, $msgid = 0) {
    ...
    foreach ($feedbacks AS $feedback) {
      ...
      elseif ($feedback->device_type == 'android') {
        ...
        self::updateStats('androidfail', $androidfail, $cronjob, $msgid);
      }
      elseif ($feedback->device_type == 'chrome') {
        ...
        self::updateStats('chromefail', $chromefail, $cronjob, $msgid);
      }
      ...
      if (!empty(self::$sendoptions['extravalue'])) {
        if (self::$sendoptions['extra_type'] == 'normal') {
          if (self::$apisetting['android_corona_payload'] == 1) {
            $aPayload['aps']['custom'] = json_encode(array('relatedvalue' => stripslashes(self::$sendoptions['extravalue'])), defined('JSON_UNESCAPED_UNICODE') ? JSON_UNESCAPED_UNICODE : 0);
          }
          else{
  ...
  public static function updateStats($index = '', $value = 0, $cronjob = false, $archiveid=0) {
    ...
    if ($index == 'all') {
      ...
      global $wpdb;
      $wpdb->update($wpdb->prefix.'push_archive', array('endtime' => date('Y-m-d H:i:s', current_time('timestamp'))), array('id' => $archid));
      ...
      if(!empty($pastvalues['views']) && isset($stats['safariviews'])){
