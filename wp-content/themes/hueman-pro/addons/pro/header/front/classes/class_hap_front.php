...
class PC_HAP_front {
    ...
    function set_hooks_and_model() {
      ...
      add_action( '__after_header'           , array( $this , 'hu_print_section_tmpl'));
      ...
      add_filter( 'ha_user_options_style', array( $this, 'ha_write_header_inline_css' ) );



      //FRONT ASSETS :
      add_action( 'wp_enqueue_scripts'       , array( $this , 'hu_enqueue_plug_resources'));
    }
    ...
    function _get_pro_header_model() {
        ...
        //If the option is set yet ( this means that it is not locally set and not set by any parents ) , return the default option
        ...
        $local_skoped_opt = HA_SKOP_OPT() -> ha_get_cached_opt( 'local', $pro_header_slider_short_opt_name );
        ...
        if ( empty( $_pre_slides ) && ( false == $local_skoped_opt || '_no_set_' == $local_skoped_opt ) ) {
            if ( hu_can_have_default_slide_title() &&  has_post_thumbnail() && $use_contextual_data_on ) {
                $_pre_slides[] = $this -> ha_get_default_contextual_slide_model();
        ...
        // ha_error_log('/////////// SLIDER MODEL');
        // ha_error_log( print_R( $_m , true ) );
    ...
    //hook : __before_main or __before_content
    function hu_print_section_tmpl() {
        // load_template( HA_BASE_PATH . 'addons/pro/header/front/tmpl/before-content-section.php', true );//true for require_once
        load_template( HA_BASE_PATH . 'addons/pro/header/front/tmpl/slider-tmpl.php', true );//true for require_once
    }


    //hook : hu_tmpl_header-main
    function hu_load_custom_header_tmpl() {
        $_full_path = HA_BASE_PATH . 'addons/pro/header/front/tmpl/header-main.php';
    ...
    function hu_enqueue_plug_resources() {
        wp_enqueue_style(
            'hph-front-style' ,
            //sprintf('%1$s/front/assets/css/hph-front%2$s.css' , HU_PRO_HEADER_BASE_URL, ( defined('WP_DEBUG') && true === WP_DEBUG ) ? '' : '.min'),
            sprintf('%1$s/addons/assets/front/css/hph-front%2$s.css' , HU_AD() -> ha_get_base_url() , ( defined('WP_DEBUG') && true === WP_DEBUG ) ? '' : '.min'),
            null,
            time(),
            $media = 'all'
        );
        wp_enqueue_script(
            'hph-flickity-js',
            sprintf('%1$s/addons/assets/front/js/vendors/flickity%2$s.js' , HU_AD() -> ha_get_base_url() , ( defined('WP_DEBUG') && true === WP_DEBUG ) ? '' : '.min'),
            null,
            time(),
            true
        );
        //if ( HU_IS_PRO_ADDONS ) {
            wp_enqueue_script(
                'hph-js',
                sprintf('%1$s/addons/assets/front/js/hph-front%2$s.js' , HU_AD() -> ha_get_base_url() , ( defined('WP_DEBUG') && true === WP_DEBUG ) ? '' : '.min'),
                array( 'hph-flickity-js', 'hu-front-scripts' ),
                time(),
                true
            );
        //}
    ...
    *
    **/
    function ha_write_header_inline_css( $_css ) {
        $model = HU_AD() -> ha_get_model( 'slider', array( PC_HAP_front::$instance , '_get_pro_header_model') );
        $slider_opts  = $model['options'];
