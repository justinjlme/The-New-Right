          ...
          <div class="tablenav top">
            ...
            <div class="alignleft actions smpush-canhide">
              <select name="device_type">
                <option value="0"><?php echo __('Show all platforms', 'smpush-plugin-lang')?></option>
                <option value="<?php echo $types_name->ios_name;?>" <?php if($_GET['device_type'] == $types_name->ios_name){?>selected="selected"<?php }?>>iOS</option>
             ...
             <table class="wp-list-table widefat fixed tags" cellspacing="0" <?php if(get_bloginfo('version') < 3.8){?>style="table-layout: auto"<?php }?>>
                <thead>
                   <tr>
                      ...
                      <th scope="col" class="manage-column smpush-center"><span><?php echo __('GPS Location', 'smpush-plugin-lang')?></span></th>
                ...
                <?php if($tokens){$counter = 0;foreach($tokens AS $token){$counter++;?>
                   <tr id="smpush-service-tab-<?php echo $token->id;?>" class="smpush-service-tab <?php if($counter%2 == 0){echo 'alternate';}?>">
                      ...
                      <td class="name column-name smpush-center"><?php if(!empty($token->latitude) && $token->latitude != '0.00000000'):?><a href="https://www.google.com/maps?q=loc:<?php echo $token->latitude?>,<?php echo $token->longitude?>" target="_blank"><img src="<?php echo smpush_imgpath.'/gps.png';?>" /></a><?php endif;?></td>
