...
class AdminNotice {
	...
	static function prefix($s){
            return App::PREFIX.$s;
        }
	
	static function getNonceAction() {
		return self::prefix(self::NONCE_SUFFIX);
	}
	...
	static protected function getDismissDataKey() {
		return self::prefix(self::USER_META_NONCE_DISMISS_SUFFIX);
	}
	...
	protected function getDismissButton() {
		return sprintf('<a href="%s" data-nonce="%s" data-action="%s" data-id="%s" class="%s" title="%s">&times;</a>',
			...
			$action = esc_attr(self::prefix('_'. self::AJAX_ACTION)),
			$id = esc_attr($this->id),
			$class = esc_attr(self::prefix('-dismiss')),
	...
	function __toString() {
		...
		wp_enqueue_script(self::prefix('-backend'));
		wp_enqueue_style(self::prefix('-backend'));
		$className = self::prefix('-admin-notice');
