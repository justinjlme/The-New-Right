...
class Mailchimp_Handler {
	...
	public function post( $end_point, $data ) {
	...
	public function get_groups( $list_id ) {
		$results = $this->query( 'lists/' . $list_id . '/interest-categories' );
		$groups = [];

		if ( ! empty( $results['categories'] ) ) {
			foreach ( $results['categories'] as $category ) {
				$interests_results = $this->query( 'lists/' . $list_id . '/interest-categories/' . $category['id'] . '/interests' );
	...
	public function get_fields( $list_id ) {
		$results = $this->query( 'lists/' . $list_id . '/merge-fields' );
