...
$(document).ready(function() {
  ...
  $('.smpushReuqestTypePicker').change(function(){
   if($(this).val() == "native"){
     $(".smpush-popup-settings").hide();
     $(".smpush-icon-settings").hide();
   }
   else if($(this).val() == "popup"){
	$(".smpush-icon-settings").hide();
	$(".smpush-popup-settings").show();
   }
   else if($(this).val() == "icon"){
     $(".smpush-popup-settings").hide();
     $(".smpush-icon-settings").show();
   }
  });
  
  if($('.smpush_emoji').length > 0){
	  $('.smpush_emoji').emojiPicker();
  }
  
  if($('.smpush_tabs').length > 0){
    $(".smpush_tabs").tabs().removeClass('ui-widget').removeClass('ui-widget-content');
    $(".smpush_tabs ul").removeClass('ui-widget').removeClass('ui-widget-header');
  }
  
  if($('.smpush_jradio').length > 0){
    $(".smpush_jradio").labelauty();
  }
  
  if($('.smpush-datepicker').length > 0){
    $(".smpush-datepicker").datepicker();
  }
  
  if($('.smpush-timepicker').length > 0){
    $('.smpush-timepicker').datetimepicker({
      timeFormat: "hh:mm tt",
      dateFormat : 'yy-mm-dd'
    });
  }
...
function smpushProcessSmilies(message){
	message = message.replace(/U\+([0-9A-F]{4,5})/g, function (match, code) {
