...
namespace com\cminds\package\pro\v1_6_2;
				...
				<td>
					...
					<?php if ( false !== $license ) : ?>
						...
						if ( !empty( $data->expires ) && $status !== false && $status == 'valid' ) {
							...
							$explanationTooltip	 = '<span class="cminds-package-show-tooltip" title="License expiration date does not mean that the plugin will stop working. Once license expires you will not be able to receive updates or support. You can always renew your license and receive a 35% discount off the plugin cost."></span>';
							echo '<p>';
							...
							if ( $expiring && $data->expires !== 'lifetime' ) {
								...
								?>
								<a href="<?php echo $renewLink; ?>" target="_blank">Renew your License and receive 35% discount</a>
								<?php
							}
							echo '</p>';
						}
...
//						}
		...
		public function check_for_license_change( $value, $old_value, $option ) {
			if ( $value !== $old_value ) {
				$this->licenseKeyChange = TRUE; //94673a916c891ffe53e0da0caccf17b3
			}
			return $value;
		}
		...
		public function showMessage() {
			...
			if ( empty( $transientMessage ) ) {
				...
				if ( $this->cmindsProPackage->getSetting( 'cminds_debug_notices_display' ) ) {
					$this->display_license_message();
		...
		public function display_license_message() {
			...
			switch ( $licenseStatus ) {
				...
				case 'expired':
					...
					. ' <a href="' . $renewLink . '" target="_blank">Renew your license here</a> and receive a 35% discount. See <a href="http://creativeminds.helpscoutdocs.com/article/402-general-support-how-to-renew-your-license" target="_blank">renewal instructions</a>';
					$this->messageError	 = TRUE;
					break;
				case 'deactivated':
				case 'failed':
				...
				case 'invalid':
					if ( 2 === $expiring ) {
						...
						. ' <a href="' . $renewLink . '" target="_blank">Renew your license here</a> and receive a 35% discount. See <a href="http://creativeminds.helpscoutdocs.com/article/402-general-support-how-to-renew-your-license" target="_blank">renewal instructions</a>. ' . $licenseInfoRefreshLink;
					} else {
						/*
						 * This license is invalid / either it has expired or the key was invalid
						 */
