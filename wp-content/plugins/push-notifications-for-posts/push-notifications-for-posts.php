...
Plugin URI: http://codecanyon.net/item/push-notifications-for-posts/6868260
...
if (class_exists('PNFW_Push_Notifications_for_WordPress') || class_exists('PNFW_Push_Notifications_for_WordPress_Lite')) {
 wp_die(
  __("To activate Push Notifications for Posts you must first disable Push Notifications for WordPress and Push Notifications for WordPress (Lite)", 'pnfw'),
...
$push_notifications_for_wordpress = new PNFW_Push_Notifications_for_Posts();

final class PNFW_Push_Notifications_for_Posts {
 ...
 function activate() {
  if (version_compare(PHP_VERSION, self::MIN_PHP_REQUIRED, '<')) {
   ...
   wp_die(
    sprintf(__("Push Notifications for Posts requires PHP version %s or later.", 'pnfw'), self::MIN_PHP_REQUIRED),
    __("Plugin Activation Error", 'pnfw'),
    array('response' => 200, 'back_link' => TRUE)
   );
  ...
  if (is_multisite()) {
   ...
   wp_die(
    __("Push Notifications for Posts is incompatible with WordPress Multisite.", 'pnfw'),
    __("Plugin Activation Error", 'pnfw'),
    array('response' => 200, 'back_link' => TRUE)
   );
 ...
 function manage_routes() {
  ...
  // Manages the old PNFP register API
  add_rewrite_rule('register/?$', 'index.php?control_action=register', 'top'); // FIXME deprecated, will be removed soon
