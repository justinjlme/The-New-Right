...
if(!c.isObject(b))throw new Error("prepareModuleForAPI : a module must be an object to be instantiated.");var d=this,e={};return c.each(d.getDefaultModuleApiModel(),function(f,g){var h=b[g];switch(g){case"id":c.isEmpty(h)?e[g]=d.generateModuleId(b.module_type):e[g]=h;break;case"module_type":if(!c.isString(h)||c.isEmpty(h))throw new Error("prepareModuleForAPI : a module type must a string not empty");e[g]=h;break;case"items":if(!c.isArray(h))throw new Error("prepareModuleForAPI : a module item list must be an array");e[g]=h;break;case"modOpt":if(!c.isObject(h))throw new Error("prepareModuleForAPI : a module modOpt property must be an object");e[g]=h;break;case"crud":if(c.has(a.czrModuleMap,b.module_type))h=a.czrModuleMap[b.module_type].crud;else if(!c.isUndefined(h)&&!c.isBoolean(h))throw new Error('prepareModuleForAPI : the module param "crud" must be a boolean');e[g]=h||!1;break;case"multi_item":if(c.has(a.czrModuleMap,b.module_type))h=a.czrModuleMap[b.module_type].crud||a.czrModuleMap[b.module_type].multi_item;else if(!c.isUndefined(h)&&!c.isBoolean(h))throw new Error('prepareModuleForAPI : the module param "multi_item" must be a boolean');e[g]=h||!1;break;case"sortable":if(c.has(a.czrModuleMap,b.module_type))h=a.czrModuleMap[b.module_type].sortable||a.czrModuleMap[b.module_type].crud||a.czrModuleMap[b.module_type].multi_item;else if(!c.isUndefined(h)&&!c.isBoolean(h))throw new Error('prepareModuleForAPI : the module param "sortable" must be a boolean');e[g]=h||!1;break;case"control":e[g]=d;break;case"section":if(!c.isString(h)||c.isEmpty(h))throw new Error("prepareModuleForAPI : a module section must be a string not empty");e[g]=h;break;case"column_id":if(!c.isString(h)||c.isEmpty(h))throw new Error("prepareModuleForAPI : a module column id must a string not empty");e[g]=h;break;case"sektion":if(!c.isObject(h)||c.isEmpty(h))throw new Error("prepareModuleForAPI : a module sektion must be an object not empty");e[g]=h;break;case"sektion_id":if(!c.isString(h)||c.isEmpty(h))throw new Error("prepareModuleForAPI : a module sektion id must be a string not empty");e[g]=h;break;case"is_added_by_user":if(!c.isUndefined(h)&&!c.isBoolean(h))throw new Error('prepareModuleForAPI : the module param "is_added_by_user" must be a boolean');e[g]=h||!1;break;case"dirty":e[g]=h||!1}}),e},generateModuleId:function(a,b,d){if(d=d||1,d>100)throw new Error("Infinite loop when generating of a module id.");var e=this;b=b||e._getNextModuleKeyInCollection();var f=a+"_"+b;if(!c.has(e,"czr_moduleCollection")||!c.isArray(e.czr_moduleCollection()))throw new Error("The module collection does not exist or is not properly set in control : "+e.id);return e.isModuleRegistered(f)?(b++,d++,e.generateModuleId(a,b,d)):f},_getNextModuleKeyInCollection:function(){var a=this,b={},d=0;return c.isEmpty(a.czr_moduleCollection())||(b=c.max(a.czr_moduleCollection(),function(a){return parseInt(a.id.replace(/[^\/\d]/g,""),10)}),d=parseInt(b.id.replace(/[^\/\d]/g,""),10)+1),d}})}(wp.customize,jQuery,_);var CZRBaseModuleControlMths=CZRBaseModuleControlMths||{};!function(a,b,c){b.extend(CZRBaseModuleControlMths,{registerModulesOnInit:function(d){var e=this,f=[];c.each(e.getSavedModules(),function(g,h){if(!d.czr_Item.has(g.sektion_id))return a.errorLog("Warning Module "+g.id+" is orphan : it has no sektion to be embedded to. It Must be removed."),void f.push(g);var i=d.czr_Item(g.sektion_id);if(c.isUndefined(i))throw new Error("sektion instance missing. Impossible to instantiate module : "+g.id);b.extend(g,{sektion:i}),e.updateModulesCollection({module:g})}),e.moduleCollectionReady.then(function(){c.isEmpty(f)||e.moduleCollectionReact(e.czr_moduleCollection(),[],{orphans_module_removal:f})})},updateModulesCollection:function(a){var d=this,e=d.czr_moduleCollection(),f=b.extend(!0,[],e);if(c.has(a,"collection"))return void d.czr_moduleCollection.set(a.collection,a.data||{});if(!c.has(a,"module"))throw new Error("updateModulesCollection, no module provided "+d.id+". Aborting");var g=d.prepareModuleForAPI(c.clone(a.module));c.findWhere(f,{id:g.id})?c.each(e,function(a,b){a.id==g.id&&(f[b]=g)}):f.push(g);var h={};c.has(a,"data")&&(h=b.extend(!0,{},a.data),b.extend(h,{module:g})),d.czr_moduleCollection.set(f,h)},moduleCollectionReact:function(d,e,f){var g=this,h=c.size(d)>c.size(e),i=c.size(e)>c.size(d);c.size(e)==c.size(d);if(is_collection_sorted=!1,i){var j=c.filter(e,function(a){return c.isUndefined(c.findWhere(d,{id:a.id}))});j=j[0],g.czr_Module.remove(j.id)}c.isObject(f)&&c.has(f,"module")&&(f.module_id=f.module.id,f.module=g.prepareModuleForDB(b.extend(!0,{},f.module))),!g.isMultiModuleControl()&&h||a(this.id).set(g.filterModuleCollectionBeforeAjax(d),f)},filterModuleCollectionBeforeAjax:function(a){var d,e=this,f=b.extend(!0,[],a);if(c.each(a,function(a,c){var d=b.extend(!0,{},a);f[c]=e.prepareModuleForDB(d)}),e.isMultiModuleControl())return f;if(c.size(a)>1)throw new Error("There should not be several modules in the collection of control : "+e.id);if(!c.isArray(a)||c.isEmpty(a)||!c.has(a[0],"items"))throw new Error("The setting value could not be populated in control : "+e.id);var g=a[0].id;if(!e.czr_Module.has(g))throw new Error("The single module control ("+e.id+") has no module registered with the id "+g);var h=e.czr_Module(g);if(!c.isArray(h().items))throw new Error("The module "+g+" should be an array in control : "+e.id);return d=h.isMultiItem()?h().items:h().items[0]||[],h.hasModOpt()?c.union([h().modOpt],d):d},prepareModuleForDB:function(a){if(!c.isObject(a))throw new Error("MultiModule Control::prepareModuleForDB : a module must be an object. Aborting.");var b=this,d={};return c.each(b.getDefaultModuleDBModel(),function(e,f){if(!c.has(a,f))throw new Error("MultiModule Control::prepareModuleForDB : a module is missing the property : "+f+" . Aborting.");var g=a[f];switch(f){case"items":if(!c.isArray(g))throw new Error("prepareModuleForDB : a module item list must be an array");d[f]=g;break;case"id":if(!c.isString(g)||c.isEmpty(g))throw new Error("prepareModuleForDB : a module id must a string not empty");d[f]=g;break;case"module_type":if(!c.isString(g)||c.isEmpty(g))throw new Error("prepareModuleForDB : a module type must a string not empty");d[f]=g;break;case"column_id":if(!c.isString(g)||c.isEmpty(g))throw new Error("prepareModuleForDB : a module column id must a string not empty");d[f]=g;break;case"sektion_id":if(!c.isObject(a.sektion)||!c.has(a.sektion,"id"))throw new Error("prepareModuleForDB : a module sektion must be an object with an id.");d[f]=a.sektion.id;break;case"dirty":if(b.czr_Module.has(a.id)?d[f]=b.czr_Module(a.id).isDirty():d[f]=g,!c.isBoolean(d[f]))throw new Error("prepareModuleForDB : a module dirty state must be a boolean.")}}),d}})}(wp.customize,jQuery,_);var CZRMultiModuleControlMths=CZRMultiModuleControlMths||{};!function(a,b,c){b.extend(CZRMultiModuleControlMths,{initialize:function(b,c){var d=this;a.consoleLog("IN MULTI MODULE INITIALIZE ? ",c),a(b).callbacks.add(function(){return d.syncColumn.apply(d,arguments)}),a.CZRBaseModuleControl.prototype.initialize.call(d,b,c)},ready:function(){var b=this;a.consoleLog("MODULE-COLLECTION CONTROL READY",this.id),a.CZRBaseModuleControl.prototype.ready.apply(b,arguments)},syncColumn:function(d,e,f){if(a.consoleLog("IN SYNC COLUMN",d,e,f),(c.isUndefined(f)||!f.silent)&&(a.consoleLog("IN SYNXXX",a.control("hu_theme_options[module-collection]").syncSektionModule()(),this.syncSektionModule()(),this.id),!c.has(f,"orphans_module_removal"))){var g=a.control(this.id),h=c.filter(d,function(a,b){return!c.findWhere(e,{id:a.id})});c.isEmpty(h)||(a.consoleLog("ADDED MODULE?",h),c.each(h,function(a){g.syncSektionModule().czr_Column(a.column_id).updateColumnModuleCollection({module:a})}));var i=c.filter(e,function(a,b){return!c.findWhere(d,{id:a.id})});c.isEmpty(i)||c.each(i,function(a){g.syncSektionModule().czr_Column(a.column_id).removeModuleFromColumnCollection(a)}),c.size(e)==c.size(d)&&c.has(f,"module")&&c.has(f,"source_column")&&c.has(f,"target_column")&&b.when(g.syncSektionModule().moveModuleFromTo(f.module,f.source_column,f.target_column)).done(function(){g.syncSektionModule().control.trigger("module-moved",{module:f.module,source_column:f.source_column,target_column:f.target_column})}),g.trigger("columns-synchronized",d)}},removeModule:function(a){var b=this;b.czr_Module.has(a.id)&&"resolved"==b.czr_Module(a.id).embedded.state()&&b.czr_Module(a.id).container.remove(),b.removeModuleFromCollection(a)},removeModuleFromCollection:function(a){var d=this,e=d.czr_moduleCollection(),f=b.extend(!0,[],e);f=c.filter(f,function(b){return b.id!=a.id}),d.czr_moduleCollection.set(f)}})}(wp.customize,jQuery,_);var CZRMultiModuleControlMths=CZRMultiModuleControlMths||{};!function(a,b,c){b.extend(CZRMultiModuleControlMths,{getMultiModuleExtender:function(d){var e=this;return b.extend(e.CZRModuleExtended,{initialize:function(c,e){var f=this;d.prototype.initialize.call(f,c,e),a.consoleLog("MODULE INSTANTIATED : ",f.id),b.extend(f,{singleModuleWrapper:"czr-single-module-wrapper",sektionModuleTitle:"czr-module-sektion-title-part",ruModuleEl:"czr-ru-module-sektion-content"}),f.czr_ModuleState=new a.Value((!1)),f.isReady.done(function(){f.setupModuleView()}),f.moduleTitleEmbedded=b.Deferred(),f.modColumn=new a.Value,f.modColumn.set(e.column_id),f.modColumn.bind(function(c,d){a.consoleLog("MODULE "+f.id+" HAS BEEN MOVED TO COLUMN",c,f());var e=f(),g=b.extend(!0,{},e);g.column_id=c,f.set(g,{target_column:c,source_column:d})})},ready:function(e){var f=this;a.consoleLog("MODULE READY IN EXTENDED MODULE CLASS : ",f.id),b.when(f.renderModuleWrapper(e)).done(function(a){if(c.isUndefined(a)||!1===a.length)throw new Error("Module container has not been embedded for module :"+f.id);f.container=a,f.embedded.resolve()}),d.prototype.ready.call(f)}}),e.CZRModuleExtended},CZRModuleExtended:{renderModuleWrapper:function(a){var c=this;if("resolved"==c.embedded.state())return c.container;if(0===b("#tmpl-"+c.singleModuleWrapper).length)throw new Error("No template for module "+c.id+". The template script id should be : #tmpl-"+c.singleModuleWrapper);var d=wp.template(c.singleModuleWrapper),e={id:c.id,type:c.module_type},f=b(d(e));return a?b.when(b(".czr-module-collection-wrapper",c._getColumn().container).find(".czr-module-candidate").after(f)).done(function(){b(".czr-module-collection-wrapper",c._getColumn().container).find(".czr-module-candidate").remove()}):b(".czr-module-collection-wrapper",c._getColumn().container).append(f),f},setupModuleView:function(){var b=this;b.view_event_map=[{trigger:"click keydown",selector:[".czr-remove-mod","."+b.control.css_attr.cancel_alert_btn].join(","),name:"toggle_remove_alert",actions:["toggleModuleRemoveAlert"]},{trigger:"click keydown",selector:"."+b.control.css_attr.remove_view_btn,name:"remove_module",actions:["removeModule"]},{trigger:"click keydown",selector:".czr-edit-mod",name:"edit_module",actions:["setModuleViewVisibility","sendEditModule"]},{trigger:"click keydown",selector:".czr-module-back",name:"back_to_column",actions:["setModuleViewVisibility"]},{trigger:"mouseenter",selector:".czr-mod-header",name:"hovering_module",actions:function(a){b.control.previewer.send("start_hovering_module",{id:b.id})}},{trigger:"mouseleave",selector:".czr-mod-header",name:"hovering_module",actions:function(a){b.control.previewer.send("stop_hovering_module",{id:b.id})}}],b.embedded.done(function(){b.czr_ModuleState.callbacks.add(function(){return b.setupModuleViewStateListeners.apply(b,arguments)}),a.CZR_Helpers.setupDOMListeners(b.view_event_map,{module:{id:b.id},dom_el:b.container},b)})},setModuleViewVisibility:function(c,d){var e=this;e.czr_ModuleState(!e.czr_ModuleState()),a.czrModulePanelState.set(!1),a.czrSekSettingsPanelState.set(!1),e.control.syncSektionModule().closeAllOtherSektions(b(c.dom_event.currentTarget,c.dom_el))},sendEditModule:function(a){var b=this;b.control.previewer.send("edit_module",{id:b.id})},setupModuleViewStateListeners:function(c){var d=this;a.czr_isModuleExpanded=a.czr_isModuleExpanded||new a.Value,c?a.czr_isModuleExpanded(d):a.czr_isModuleExpanded(!1),b.when(d.toggleModuleViewExpansion(c)).done(function(){c?(d.renderModuleTitle(),d.populateSavedItemCollection()):d.czr_Item.each(function(a){a.viewState.set("closed"),a._destroyView(0),d.czr_Item.remove(a.id)})})},renderModuleTitle:function(){var a=this;if("resolved"!=a.moduleTitleEmbedded.state()){if(0===b("#tmpl-"+a.sektionModuleTitle).length)throw new Error("No sektion title Module Part template for module "+a.id+". The template script id should be : #tmpl-"+a.sektionModuleTitle);b.when(b(a.container).find(".czr-mod-content").prepend(b(wp.template(a.sektionModuleTitle)({id:a.id})))).done(function(){a.moduleTitleEmbedded.resolve()})}},toggleModuleViewExpansion:function(a,c){var d=this;b(".czr-mod-content",d.container).slideToggle({duration:c||200,done:function(){var b=d.container.closest(".wp-full-overlay"),c=d.container.find(".czr-module-back"),e=d.container.find(".czr-module-title");d.container.toggleClass("open",a),b.toggleClass("czr-module-open",a),e.attr("tabindex",a?"-1":"0"),c.attr("tabindex",a?"0":"-1"),a?c.focus():e.focus(),a&&d._adjustScrollExpandedBlock(d.container)}})},toggleModuleRemoveAlert:function(a){var d=this,e=this.control,f=b("."+d.control.css_attr.remove_alert_wrapper,d.container).first(),g=(a.dom_event,e.syncSektionModule().czr_Column(d.column_id).container);if(c.has(d,"preItem")&&e.syncSektionModule().preItemExpanded.set(!1),b("."+d.control.css_attr.remove_alert_wrapper,g).not(f).each(function(){b(this).hasClass("open")&&b(this).slideToggle({duration:200,done:function(){b(this).toggleClass("open",!1),b(this).siblings().find("."+d.control.css_attr.display_alert_btn).toggleClass("active",!1)}})}),!wp.template(d.AlertPart)||!d.container)throw new Error("No removal alert template available for module :"+d.id);f.html(wp.template(d.AlertPart)({title:d().title||d.id})),f.slideToggle({duration:200,done:function(){var c=!b(this).hasClass("open")&&b(this).is(":visible");b(this).toggleClass("open",c),b(a.dom_el).find("."+d.control.css_attr.display_alert_btn).toggleClass("active",c),c&&d._adjustScrollExpandedBlock(d.container)}})},removeModule:function(a){this.control.removeModule(a.module)},_getColumn:function(){var a=this;return a.control.syncSektionModule().czr_Column(a.modColumn())},_getSektion:function(){}}})}(wp.customize,jQuery,_);var CZRMultiplePickerMths=CZRMultiplePickerMths||{};!function(a,b,c){b.extend(CZRMultiplePickerMths,{ready:function(){function a(a){return a.text.replace(/\u2013|\u2014/g,"")}var c=this,d=this.container.find("select");d.select2({closeOnSelect:!1,templateSelection:a}),d.on("change",function(a){0===b(this).find("option:selected").length&&c.setting.set([])})}})}(wp.customize,jQuery,_);var CZRCroppedImageMths=CZRCroppedImageMths||{};!function(a,b,c){"function"==typeof wp.media.controller.Cropper&&"function"==typeof a.CroppedImageControl&&(wp.media.controller.CZRCustomizeImageCropper=wp.media.controller.Cropper.extend({doCrop:function(a){var b=a.get("cropDetails"),c=this.get("control");return b.dst_width=c.params.dst_width,b.dst_height=c.params.dst_height,wp.ajax.post("crop-image",{wp_customize:"on",nonce:a.get("nonces").edit,id:a.get("id"),context:c.id,cropDetails:b})}}),b.extend(CZRCroppedImageMths,{initFrame:function(){var a=_wpMediaViewsL10n;this.frame=wp.media({button:{text:a.select,close:!1},states:[new wp.media.controller.Library({title:this.params.button_labels.frame_title,library:wp.media.query({type:"image"}),multiple:!1,date:!1,priority:20,suggestedWidth:this.params.width,suggestedHeight:this.params.height}),new wp.media.controller.CZRCustomizeImageCropper({imgSelectOptions:this.calculateImageSelectOptions,control:this})]}),this.frame.on("select",this.onSelect,this),this.frame.on("cropped",this.onCropped,this),this.frame.on("skippedcrop",this.onSkippedCrop,this)},onSelect:function(){var a=this.frame.state().get("selection").first().toJSON();return a.mime&&a.mime.indexOf("image")>-1?void(c.contains(["image/svg+xml","image/gif"],a.mime)||this.params.width===a.width&&this.params.height===a.height&&!this.params.flex_width&&!this.params.flex_height?(this.setImageFromAttachment(a),this.frame.close()):this.frame.setState("cropper")):void this.frame.trigger("content:error")}}))}(wp.customize,jQuery,_);var CZRUploadMths=CZRUploadMths||{};!function(a,b,c){b.extend(CZRUploadMths,{ready:function(){var c=this;this.params.removed=this.params.removed||"",this.success=b.proxy(this.success,this),this.uploader=b.extend({container:this.container,browser:this.container.find(".czr-upload"),success:this.success,plupload:{},params:{}},this.uploader||{}),c.params.extensions&&(c.uploader.plupload.filters=[{title:a.l10n.allowedFiles,extensions:c.params.extensions}]),c.params.context&&(c.uploader.params["post_data[context]"]=this.params.context),a.settings.theme.stylesheet&&(c.uploader.params["post_data[theme]"]=a.settings.theme.stylesheet),this.uploader=new wp.Uploader(this.uploader),this.remover=this.container.find(".remove"),this.remover.on("click keydown",function(a){"keydown"===a.type&&13!==a.which||(c.setting.set(c.params.removed),a.preventDefault())}),this.removerVisibility=b.proxy(this.removerVisibility,this),this.setting.bind(this.removerVisibility),this.removerVisibility(this.setting())},success:function(a){this.setting.set(a.get("id"))},removerVisibility:function(a){this.remover.toggle(a!=this.params.removed)}})}(wp.customize,jQuery,_);var CZRLayoutSelectMths=CZRLayoutSelectMths||{};!function(a,b,c){b.extend(CZRLayoutSelectMths,{ready:function(){this.setupSelect()},setupSelect:function(a){function d(a){if(!a.id)return a.text;if(c.has(e.params.layouts,a.element.value)){var d=e.params.layouts[a.element.value],f=d.src,g=d.label,h=b('<img src="'+f+'" class="czr-layout-img" title="'+g+'" /><span class="czr-layout-title">'+g+"</span>");return h}}var e=this;$_select=this.container.find("select"),$_select.select2({templateResult:d,templateSelection:d,minimumResultsForSearch:1/0})}})}(wp.customize,jQuery,_),function(a,b,c){b.extend(CZRBaseControlMths,a.Events),b.extend(a.Control.prototype,a.Events),b.extend(CZRModuleMths,a.Events),b.extend(CZRItemMths,a.Events),b.extend(CZRModOptMths,a.Events),b.extend(CZRSkopeBaseMths,a.Events),b.extend(CZRSkopeMths,a.Events),b.extend(CZRBaseControlMths,a.CZR_Helpers),b.extend(CZRInputMths,a.CZR_Helpers),b.extend(CZRModuleMths,a.CZR_Helpers),b.extend(CZRSkopeMths,a.CZR_Helpers),a.CZR_skopeBase=a.Class.extend(CZRSkopeBaseMths),a.CZR_skopeSave=a.Class.extend(CZRSkopeSaveMths),a.CZR_skopeReset=a.Class.extend(CZRSkopeResetMths),a.CZR_skope=a.Value.extend(CZRSkopeMths),c.has(a,"HeaderTool")&&(a.czr_HeaderTool=b.extend(!0,{},a.HeaderTool)),a.CZRInput=a.Value.extend(CZRInputMths),a.CZRItem=a.Value.extend(CZRItemMths),a.CZRModOpt=a.Value.extend(CZRModOptMths),a.CZRModule=a.Value.extend(CZRModuleMths),a.CZRDynModule=a.CZRModule.extend(CZRDynModuleMths),c.isUndefined(window.CZRColumnMths)||(a.CZRColumn=a.Value.extend(CZRColumnMths)),a.CZRBaseControl=a.Control.extend(CZRBaseControlMths),a.CZRBaseModuleControl=a.CZRBaseControl.extend(CZRBaseModuleControlMths),a.CZRMultiModuleControl=a.CZRBaseModuleControl.extend(CZRMultiModuleControlMths),a.CZRUploadControl=a.Control.extend(CZRUploadMths),a.CZRLayoutControl=a.Control.extend(CZRLayoutSelectMths),a.CZRMultiplePickerControl=a.Control.extend(CZRMultiplePickerMths),b.extend(a.controlConstructor,{czr_upload:a.CZRUploadControl,czr_module:a.CZRBaseModuleControl,czr_multi_module:a.CZRMultiModuleControl,czr_multiple_picker:a.CZRMultiplePickerControl,czr_layouts:a.CZRLayoutControl}),"function"==typeof a.CroppedImageControl&&(a.CZRCroppedImageControl=a.CroppedImageControl.extend(CZRCroppedImageMths),b.extend(a.controlConstructor,{czr_cropped_image:a.CZRCroppedImageControl})),a.czrInputMap=a.czrInputMap||{},b.extend(a.czrInputMap,{text:"",textarea:"",check:"setupIcheck",select:"setupSelect",number:"setupStepper",upload:"setupImageUploader",color:"setupColorPicker",content_picker:"setupContentPicker",text_editor:"setupTextEditor",password:"",range_slider:"setupRangeSlider"})}(wp.customize,jQuery,_),function(a,b,c){var d=serverControlParams.i18n||{};a.czr_CrtlDependenciesReady=b.Deferred(),a.bind("ready",function(){c.has(a,"czr_ctrlDependencies")||(serverControlParams.isSkopOn?"resolved"!=a.czr_skopeReady.state()&&a.czr_skopeReady.done(function(){a.czr_ctrlDependencies=new a.CZR_ctrlDependencies,a.czr_CrtlDependenciesReady.resolve()}):(a.czr_ctrlDependencies=new a.CZR_ctrlDependencies,a.czr_CrtlDependenciesReady.resolve()))}),a.CZR_ctrlDependencies=a.Class.extend({dominiDeps:[],initialize:function(){var b=this;if(this.defaultDominusParams={dominus:"",servi:[],visibility:null,actions:null,onSectionExpand:!0},this.dominiDeps=c.extend(this.dominiDeps,this._getControlDeps()),!c.isArray(b.dominiDeps))throw new Error("Visibilities : the dominos dependency array is not an array.");a.czr_activeSectionId.bind(function(d){if(!c.isEmpty(d)&&a.section.has(d))try{b.setServiDependencies(d)}catch(e){a.errorLog("In api.CZR_ctrlDependencies : "+e)}}),a.bind("awaken-section",function(d){if(serverControlParams.isSkopOn&&c.has(a,"czr_skopeBase"))a.czr_skopeBase.processSilentUpdates({candidates:{},section_id:d.target,refresh:!1}).then(function(){try{b.setServiDependencies(d.target,d.source)}catch(c){a.errorLog("On awaken-section, ctrl deps : "+c)}});else try{b.setServiDependencies(d.target,d.source)}catch(e){a.errorLog("On awaken-section, ctrl deps : "+e)}}),this._handleFaviconNote()},setServiDependencies:function(d,e,f){var g=this,h=b.Deferred();if(f=f||!1,c.isUndefined(d)||!a.section.has(d))throw new Error("Control Dependencies : the targetSectionId is missing or not registered : "+d);if(a.section(d).czr_ctrlDependenciesReady=a.section(d).czr_ctrlDependenciesReady||b.Deferred(),!f&&"resolved"==a.section(d).czr_ctrlDependenciesReady.state())return h.resolve().promise();c.each(g.dominiDeps,function(b){if(!c.has(b,"dominus")||!c.isString(b.dominus)||c.isEmpty(b.dominus))throw new Error("Control Dependencies : a dominus control id must be a not empty string.");var e=a.CZR_Helpers.build_setId(b.dominus);a.control.has(e)&&a.control(e).section()==d&&(b=g._prepareDominusParams(b),c.isEmpty(b)||g._processDominusCallbacks(b.dominus,b,f).fail(function(){a.consoleLog("self._processDominusCallbacks fail for section "+d),h.reject()}).done(function(){h.resolve()}))});var i=a.CZR_Helpers.getSectionControlIds(d),j=function(b){var d=[];return c.each(g.dominiDeps,function(e){if(!c.has(e,"servi")||!c.isArray(e.servi)||!c.has(e,"dominus")||c.isEmpty(e.dominus))return void a.errorLog("Control Dependencies : wrong params in _getServusDomini.");if(c.contains(e.servi,b)&&!c.contains(d,e.dominus)){if(e=g._prepareDominusParams(e),c.isEmpty(e))return;d.push(e.dominus)}}),c.isArray(d)?d:[]},k=[];return c.each(i,function(a){c.isEmpty(j(a))||(k=c.union(k,j(a)))}),c.each(k,function(b){var c=a.CZR_Helpers.build_setId(b);a.control(c).section()!=d&&e!=a.control(c).section()&&a.trigger("awaken-section",{target:a.control(c).section(),source:d})}),h.always(function(){a.section(d).czr_ctrlDependenciesReady.resolve()}),h.promise()},_deferCallbackForControl:function(d,e,f){var g=b.Deferred();if(c.isEmpty(d)||!c.isString(d))throw new Error("_deferCallbackForControl : the control id is missing.");if(!c.isFunction(e))throw new Error("_deferCallbackForControl : callback must be a funtion.");return f=c.isUndefined(f)||!c.isArray(f)?[]:f,a.control.has(d)?"resolved"==a.control(d).deferred.embedded.state()?b.when(e.apply(null,f)).fail(function(){g.reject()}).done(function(){g.resolve()}):a.control(d).deferred.embedded.then(function(){b.when(e.apply(null,f)).fail(function(){g.reject()}).done(function(){g.resolve()})}):a.control.when(d,function(){a.control(d).deferred.embedded.then(function(){b.when(e.apply(null,f)).fail(function(){g.reject()}).done(function(){g.resolve()})})}),g.promise()},_processDominusCallbacks:function(d,e,f){var g=this,h=a.CZR_Helpers.build_setId(d),i=a(h),j=b.Deferred(),k=!1;return c.each(e.servi,function(b){if(a.control.has(a.CZR_Helpers.build_setId(b))){var d=function(a,b,d,e){var f=arguments;c.each(d,function(a,b){switch(b){case"visibility":g._setVisibility.apply(null,f);break;case"actions":c.isFunction(a)&&a.apply(null,f)}})},f=function(c){c=c||i();var f=a.CZR_Helpers.build_setId(b);g._deferCallbackForControl(f,d,[c,b,e]).always(function(){k=!0}).fail(function(){j.reject()}).done(function(){j.resolve()})};f(),c.has(i,"czr_visibilityServi")||(i.czr_visibilityServi=new a.Value([]));var h=i.czr_visibilityServi();c.contains(h,b)||(i.bind(function(a){f(a)}),i.czr_visibilityServi(c.union(h,[b])))}}),k?j.promise():j.resolve().promise()},_setVisibility:function(b,d,e,f){var g=a.CZR_Helpers.build_setId(d),h=e.visibility(b,d,e.dominus);if(f=f||!1,c.isBoolean(h)&&("unchanged"!=h||f)){var i=function(){a.state.has("silent-update-processing")&&a.state("silent-update-processing")()||(a.control(g,function(a){var b={duration:"fast",completeCallback:function(){},unchanged:!1};c.has(a,"active")&&(h=h&&a.active()),c.has(a,"defaultActiveArguments")&&(b=control.defaultActiveArguments),a.onChangeActive(h,a.defaultActiveArguments)}),a.state.has("silent-update-processing")&&a.state("silent-update-processing").unbind(i))};a.state.has("silent-update-processing")&&a.state("silent-update-processing")()?a.state("silent-update-processing").bind(i):i()}},_getControlDeps:function(){return{}},_prepareDominusParams:function(b){var d=this,e={};if(!c.isObject(b))return a.errorLog("Visibilities : a dominus param definition must be an object."),e;if(!c.has(b,"visibility")&&!c.has(b,"actions"))return a.errorLog("Visibilities : a dominus definition must include a visibility or an actions callback."),e;if(!c.has(b,"dominus")||!c.isString(b.dominus)||c.isEmpty(b.dominus))return a.errorLog("Visibilities : a dominus control id must be a not empty string."),e;var f=a.CZR_Helpers.build_setId(b.dominus);return a.control.has(f)?!c.has(b,"servi")||c.isUndefined(b.servi)||!c.isArray(b.servi)||c.isEmpty(b.servi)?(a.errorLog("Visibilities : servi must be set as an array not empty."),e):(c.each(d.defaultDominusParams,function(a,d){var f=b[d];switch(d){case"visibility":if(!c.isUndefined(f)&&!c.isEmpty(f)&&!c.isFunction(f))throw new Error("Visibilities : a dominus visibility callback must be a function : "+b.dominus);break;case"actions":if(!c.isUndefined(f)&&!c.isEmpty(f)&&!c.isFunction(f))throw new Error("Visibilities : a dominus actions callback must be a function : "+b.dominus);break;case"onSectionExpand":if(!c.isUndefined(f)&&!c.isEmpty(f)&&!c.isBoolean(f))throw new Error("Visibilities : a dominus onSectionExpand param must be a boolean : "+b.dominus)}e[d]=f}),e):(a.errorLog("Visibilities : a dominus control id is not registered : "+f),e)},_handleFaviconNote:function(){var b=this,c=a.CZR_Helpers.build_setId(serverControlParams.faviconOptionName);if(!(!a.has("site_icon")||!a.control("site_icon")||a.has(c)&&0===+a(c)()||+a("site_icon")()>0)){var e=a.control("site_icon").params.description;_newDes=["<strong>",d.faviconNote||"","</strong><br/><br/>"].join("")+e,b._printFaviconNote(_newDes),a("site_icon").callbacks.add(function(d){+d>0?(a.control("site_icon").container.find(".description").text(e),a.has(c)&&a(c).set("")):b._printFaviconNote(_newDes)})}},_printFaviconNote:function(b){a.control("site_icon").container.find(".description").html(b)}})}(wp.customize,jQuery,_),function(a,b){b(function(b){function c(a){var c=a.siblings(".open");0!==c.length&&c.offset().top<0&&b(".wp-full-overlay-sidebar-content").animate({scrollTop:-b("#customize-theme-controls").offset().top-c.height()+a.offset().top+b(".wp-full-overlay-sidebar-content").offset().top},700)}var d=a.customize||d;b(".accordion-section").not(".control-panel").click(function(){c(b(this))}),d.czrSetupCheckbox=function(a,c){b("input[type=checkbox]",d.control(a).container).each(function(){0===b(this).val()||"0"==b(this).val()||"off"==b(this).val()||_.isEmpty(b(this).val())?b(this).prop("checked",!1):b(this).prop("checked",!0),0===b(this).closest('div[class^="icheckbox"]').length&&b(this).iCheck({checkboxClass:"icheckbox_flat-grey",radioClass:"iradio_flat-grey"}).on("ifChanged",function(a){b(this).val(!1===b(this).is(":checked")?0:1),b(a.currentTarget).trigger("change")})})},d.czrSetupSelect=function(a,c){b("select[data-customize-setting-link]",d.control(a).container).not(".no-selecter-js").each(function(){b(this).selecter({})})},d.czrSetupStepper=function(a,c){b('input[type="number"]',d.control(a).container).each(function(){b(this).stepper()})},d.control.each(function(a){_.has(a,"id")&&("widget_"!=a.id.substring(0,"widget_".length)&&"nav_menu"!=a.id.substring(0,"nav_menu".length)&&d.czrSetupCheckbox(a.id),"nav_menu_locations"!=a.id.substring(0,"nav_menu_locations".length)&&d.czrSetupSelect(a.id),d.czrSetupStepper(a.id))});var e=function(){var a=b("<span/>",{"class":"customize-controls-home fa fa-home",html:'<span class="screen-reader-text">Home</span>'});b.when(b("#customize-header-actions").append(a)).done(function(){a.keydown(function(a){9!==a.which&&(13===a.which&&this.click(),a.preventDefault())}).on("click.customize-controls-home",function(){d.section.has(d.czr_activeSectionId())?d.section(d.czr_activeSectionId()).expanded(!1):d.section.each(function(a){a.expanded(!1)}),d.panel.each(function(a){a.expanded(!1)})})})};e()})}(wp,jQuery);var CZRFeaturedPageModuleMths=CZRFeaturedPageModuleMths||{};!function(a,b,c){b.extend(CZRFeaturedPageModuleMths,{initialize:function(c,d){var e=this;a.CZRDynModule.prototype.initialize.call(e,c,d),b.extend(e,{itemPreAddEl:"czr-module-fp-pre-add-view-content",itemInputList:"czr-module-fp-view-content"}),e.inputConstructor=a.CZRInput.extend(e.CZRFeaturedPagesInputMths||{}),e.itemConstructor=a.CZRItem.extend(e.CZRFeaturedPagesItem||{}),this.defaultItemModel={id:"",title:"","fp-post":"","fp-title":"","fp-text":"","fp-image":""},this.itemAddedMessage=serverControlParams.i18n.featuredPageAdded,a.section(e.control.section()).expanded.bind(function(a){"resolved"!=e.isReady.state()&&e.ready()})},addItem:function(d){var e=this,f=e.preItem,g=f();if(c.isEmpty(g)||!c.isObject(g))throw new Error("addItem : an item should be an object and not empty. In : "+e.id+". Aborted.");var h=g["fp-post"];if("undefined"!=typeof h){h=h[0];var i=function(c){f.set(b.extend(g,c)),a.CZRDynModule.prototype.addItem.call(e,d)};e.CZRFeaturedPagesItem.setContentAjaxInfo(h.id,{},i)}},CZRFeaturedPagesInputMths:{ready:function(){var b=this;b.bind("fp-post:changed",function(){b.updateItemModel()}),b.bind("fp-title:changed",function(){b.updateItemTitle()}),a.CZRInput.prototype.ready.call(b)},setupImageUploader:function(){var b=this;b.container.bind("fp-image:content_rendered",function(){b.addResetDefaultButton()}),b.container.on("click keydown",".default-fpimage-button",function(){b.setThumbnailAjax()}),a.CZRInput.prototype.setupImageUploader.call(b)},updateItemModel:function(a){var d=this,e=this.input_parent,f=c.has(d,"is_preItemInput")&&d.is_preItemInput;if(c.has(e(),"fp-post")&&!c.isEmpty(e()["fp-post"])){var g=c.clone(e()),h=g["fp-post"][0],i=h.title;f?d.module.preItemInput:e.czr_Input;if(f)b.extend(g,{title:i,"fp-title":i}),e.set(g);else{var j=function(a){c.each(a,function(a,b){e.czr_Input(b).set(a)})};e.setContentAjaxInfo(h.id,{"fp-title":i},j)}}},updateItemTitle:function(a){var d=this,e=this.input_parent,f=c.has(d,"is_preItemInput")&&d.is_preItemInput;if(!f){var g=c.clone(e()),h="undefined"!=typeof g["fp-title"]?g["fp-title"]:"";b.extend(g,{title:h}),e.set(g)}},setThumbnailAjax:function(){var a,c=this.input_parent,d=c.czr_Input("fp-post")();"undefined"!=typeof d&&(d=d[0],a=d.id,b(".fpimage-reset-messages p").hide(),request=wp.ajax.post("get-fp-post-tb",{wp_customize:"on",id:a,CZRFPNonce:serverControlParams.CZRFPNonce}),request.done(function(a){var d=a,e=c.czr_Input("fp-image");0!==d.length?(b(".fpimage-reset-messages .success",e.container).show("fast").fadeOut(),
e.set(d)):b(".fpimage-reset-messages .warning",e.container).show("fast").delay(2e3).fadeOut()}),request.fail(function(a){"undefined"!=typeof console&&console.error&&console.error(a)}))},addResetDefaultButton:function(a){var c=this,d=(c.input_parent,serverControlParams.i18n.featuredPageImgReset),e=serverControlParams.i18n.featuredPageResetSucc,f=serverControlParams.i18n.featuredPageResetErr,g='<div class="fpimage-reset-messages" style="clear:both"><p class="success" style="display:none">'+e+'</p><p class="warning" style="display:none">'+f+"</p></div>";b(".actions",c.container).append('<button type="button" class="button default-fpimage-button">'+d+"</button>"),b(".fpimage-reset-messages",c.container).detach(),b(c.container).append(g)}},CZRFeaturedPagesItem:{setContentAjaxInfo:function(a,c,d){var e=c||{};return request=wp.ajax.post("get-fp-post",{wp_customize:"on",id:a,CZRFPNonce:serverControlParams.CZRFPNonce}),request.done(function(a){var c=a.post_info;0!==c.length&&(b.extend(e,{"fp-image":c.thumbnail,"fp-text":c.excerpt}),"function"==typeof d&&d(e))}),request.fail(function(a){"undefined"!=typeof console&&console.error&&console.error(a)}),request},writeItemViewTitle:function(c){var d=this,e=d.module,f=c||d(),g=f.title?f.title:serverControlParams.i18n.featuredPageTitle;g=a.CZR_Helpers.truncate(g,25),b("."+e.control.css_attr.item_title,d.container).html(g)}}})}(wp.customize,jQuery,_);var CZRTextModuleMths=CZRTextModuleMths||{};!function(a,b,c){b.extend(CZRTextModuleMths,{initialize:function(c,d){var e=this;a.CZRModule.prototype.initialize.call(e,c,d),b.extend(e,{itemInputList:"czr-module-text-view-content"}),e.defaultItemModel={id:"",text:""}}})}(wp.customize,jQuery,_);var CZRSlideModuleMths=CZRSlideModuleMths||{};!function(a,b,c){b.extend(CZRSlideModuleMths,{initialize:function(d,e){var f=this;f.initialConstrucOptions=b.extend(!0,{},e),a.CZRDynModule.prototype.initialize.call(f,d,e),b.extend(f,{itemPreAddEl:"czr-module-slide-pre-item-input-list",itemInputList:"czr-module-slide-item-input-list",modOptInputList:"czr-module-slide-mod-opt-input-list"}),this.sliderSkins=serverControlParams.slideModuleParams.sliderSkins,f.inputConstructor=a.CZRInput.extend(f.CZRSliderItemInputCtor||{}),f.inputModOptConstructor=a.CZRInput.extend(f.CZRSliderModOptInputCtor||{}),b.extend(f.inputOptions,{content_picker:{post:"",taxonomy:""}}),f.itemConstructor=a.CZRItem.extend(f.CZRSliderItemCtor||{}),f.modOptConstructor=a.CZRModOpt.extend(f.CZRSliderModOptCtor||{}),this.defaultModOptModel=c.extend(serverControlParams.slideModuleParams.defaultModOpt,{module_id:f.id}),this.defaultItemModel=c.omit(serverControlParams.slideModuleParams.defaultSlideMod,"slide-src"),this.itemAddedMessage=serverControlParams.i18n.mods.slider["New Slide created ! Scroll down to edit it."],c.has(a,"czr_activeSectionId")&&f.control.section()==a.czr_activeSectionId()&&"resolved"!=f.isReady.state()&&f.ready(),a.section(f.control.section()).expanded.bind(function(a){"resolved"!=f.isReady.state()&&f.ready()}),f.isReady.then(function(){var d=function(b){a.CZR_Helpers.getControlSettingId(f.control.id);f.initializeModuleModel(f.initialConstrucOptions,b).done(function(a){f.set(a,{silent:!0}),f.refreshItemCollection()}).always(function(a){})},e=function(){var d=b("."+f.control.css_attr.open_pre_add_btn,f.container),e=b("."+f.control.css_attr.pre_add_wrapper,f.container),g="local"==a.czr_skope(a.czr_activeSkopeId())().skope;if(d.toggle(g),e.toggle(g),f.itemsWrapper.toggle(g),g)1==f.control.container.find(".slide-mod-skope-notice").length&&f.control.container.find(".slide-mod-skope-notice").remove();else{var h=c.findWhere(a.czr_currentSkopesCollection(),{skope:"local"}).id;f.control.container.find(".slide-mod-skope-notice").length?f.control.container.find(".slide-mod-skope-notice").show():f.control.container.append(b("<div/>",{"class":"slide-mod-skope-notice",html:[serverControlParams.i18n.mods.slider["You can set the global options of the slider here by clicking on the gear icon : height, font size, effects..."],serverControlParams.i18n.mods.slider["Those settings will be inherited by the more specific options levels."],"<br/><br/>",serverControlParams.i18n.mods.slider["Switch to the most specific level of options to start building a slider"],":",a.czr_skopeBase.buildSkopeLink(h)].join(" ")}))}};a.czr_wpQueryInfos.bind(function(a){d(a)}),a.bind("skope-switched-done",function(b){d(a.czr_wpQueryInfos()),c.delay(function(){e()},200)}),a.czr_skopeReady.then(function(){c.delay(function(){e()},200),f.callbacks.add(function(b,c){f.czr_Item.each(function(a){"expanded"==a.viewState()&&1==a.container.find(".refresh-button").length&&a.container.find(".refresh-button").prop("disabled",!1)}),f.czr_ModOpt&&f.czr_ModOpt.isReady&&f.czr_ModOpt.isReady.then(function(){a.czr_ModOptVisible()&&1==f.czr_ModOpt.container.find(".refresh-button").length&&f.czr_ModOpt.container.find(".refresh-button").prop("disabled",!1)})})})});var g=function(){f.czr_Item.each(function(a){a.writeItemViewTitle()})};f.bind("item-collection-sorted",g),f.bind("item-removed",g)},initializeModuleModel:function(d,e){var f=this,g=b.Deferred();return a.control.when(f.control.id,function(){var h=a.CZR_Helpers.getControlSettingId(f.control.id);return a.has(h)?void a.czr_skopeReady.then(function(){var h=a.czr_skope.has(a.czr_activeSkopeId())&&"local"==a.czr_skope(a.czr_activeSkopeId())().skope;if(_isLocalAndDirty=h&&f._isSettingDirty(),_isLocalAndDirty)return g.resolve(d).promise();if(!h){var i=b.extend(!0,{},d);return i.items=[],g.resolve(i).promise()}a.czr_wpQueryDataReady.then(function(h){h=a.czr_wpQueryInfos()||h;var i,j;i=c.isUndefined(e)?c.isObject(h)?h.query_data:{}:c.isObject(e)?e.query_data:{},j=b.extend(!0,{},f.defaultItemModel),d.items=[b.extend(j,{id:"default_item_"+f.id,is_default:!0,"slide-background":c.isEmpty(i.post_thumbnail_id)?"":i.post_thumbnail_id,"slide-title":c.isEmpty(i.post_title)?"":i.post_title,"slide-subtitle":c.isEmpty(i.subtitle)?"":i.subtitle})],g.resolve(d)})}):g.resolve(d).promise()}),c.delay(function(){a.control.has(f.control.id)||(a.errorLog("Slide Module : initializeModuleModel, the control has not been registered after too long."),g.resolve(d))},5e3),g.promise()},_getServerDefaultSlideItem:function(){},slideModSetupSelect:function(){if("skin"==this.id||"slide-skin"==this.id){var a=this,d=a.input_parent,e=a.module,f=e.sliderSkins,g=d();c.each(f,function(c,d){var e={value:d,html:c};d==g[a.id]&&b.extend(e,{selected:"selected"}),b('select[data-type="'+a.id+'"]',a.container).append(b("<option>",e))}),b('select[data-type="'+a.id+'"]',a.container).selecter()}},slideModSetupColorPicker:function(){var d=this,e=d.input_parent;e();d.container.find("input").iris({palettes:!0,hide:!1,change:function(d,e){var f=a.CZR_Helpers.hexToRgb(e.color.toString()),g=c.isString(f)&&-1!==f.indexOf("rgb(");g||(f="rgb(34,34,34)"),b(this).val(f).trigger("colorpickerchange").trigger("change")}})},_isCustomLink:function(a){return c.isObject(a)&&"_custom_"===a.id},_isChecked:function(a){return 0!==a&&"0"!==a&&!1!==a&&"off"!==a},_isSettingDirty:function(){if("pending"==a.czr_skopeReady.state())return!1;var b=this,c=a.CZR_Helpers.getControlSettingId(b.control.id);return a.czr_skope(a.czr_activeSkopeId()).getSkopeSettingDirtyness(c)||a.czr_skope(a.czr_activeSkopeId()).hasSkopeSettingDBValues(c)}})}(wp.customize,jQuery,_);var CZRSlideModuleMths=CZRSlideModuleMths||{};!function(a,b,c){b.extend(CZRSlideModuleMths,{CZRSliderItemInputCtor:{ready:function(){var b=this;"slide-title"===b.id&&b.bind(function(a){b.updateItemTitle(a)}),"slide-link"==b.id&&(b.defaultContentPickerOption=[{id:"_custom_",title:['<span style="font-weight:bold">',serverControlParams.i18n.mods.slider["Set a custom url"],"</span>"].join(""),type_label:"",object_type:"",url:""}]),a.CZRInput.prototype.ready.call(b)},setupSelect:function(){return this.module.slideModSetupSelect.call(this)},setupColorPicker:function(){return this.module.slideModSetupColorPicker.call(this)},updateItemTitle:function(a){var d=this,e=d.input_parent,f=(c.has(d,"is_preItemInput")&&d.is_preItemInput,b.extend(!0,{},e()));b.extend(f,{title:a}),e.set(f,{input_changed:"title",input_transport:"postMessage",not_preview_sent:!0})}},CZRSliderModOptInputCtor:{ready:function(){var b=this;"fixed-link"==b.id&&(b.defaultContentPickerOption=[{id:"_custom_",title:['<span style="font-weight:bold">',serverControlParams.i18n.mods.slider["Set a custom url"],"</span>"].join(""),type_label:"",object_type:"",url:""}]),a.CZRInput.prototype.ready.call(b)},setupSelect:function(){return this.module.slideModSetupSelect.call(this)},setupColorPicker:function(){return this.module.slideModSetupColorPicker.call(this)}}})}(wp.customize,jQuery,_);var CZRSlideModuleMths=CZRSlideModuleMths||{};!function(a,b,c){b.extend(CZRSlideModuleMths,{CZRSliderItemCtor:{ready:function(){var d=this,e=d.module;d.inputCollection.bind(function(f){if(!c.isEmpty(f)){try{d.setInputVisibilityDeps()}catch(g){a.errorLog("item.setInputVisibilityDeps() : "+g)}d.setModOptDependantsVisibilities(),d().is_default&&d._isSinglePost()&&d._printPostMetasNotice(),d.container.find(".refresh-button").prop("disabled",!0),a.CZR_Helpers.setupDOMListeners([{trigger:"click keydown",selector:".refresh-button",actions:function(b){a.previewer.refresh().done(function(){c.delay(function(){d.container.find(".refresh-button").prop("disabled",!0)},250)})}},{trigger:"click keydown",selector:".focus-button",actions:function(c){a.previewer.send("slide_focus",{module_id:d.module.id,module:{items:b.extend(!0,{},e().items),modOpt:e.hasModOpt()?b.extend(!0,{},e().modOpt):{}},item_id:d.id})}}],{model:d(),dom_el:d.container},d)}}),d.viewState.bind(function(c){"expanded"==c&&a.previewer.send("item_expanded",{module_id:d.module.id,module:{items:b.extend(!0,{},e().items),modOpt:e.hasModOpt()?b.extend(!0,{},e().modOpt):{}},item_id:d.id})}),a.CZRItem.prototype.ready.call(d)},_isSinglePost:function(){return a.czr_wpQueryInfos&&a.czr_wpQueryInfos().conditional_tags&&a.czr_wpQueryInfos().conditional_tags.is_single},_printPostMetasNotice:function(){var c=this;a.CZR_Helpers.setupDOMListeners([{trigger:"click keydown",selector:".open-post-metas-option",name:"toggle_mod_option",actions:function(){a.czr_ModOptVisible(!0,{module:c.module,focus:"section-topline-2"})}}],{model:c(),dom_el:c.container},c);var d=["<strong>",serverControlParams.i18n.mods.slider["You can display or hide the post metas ( categories, author, date ) in"],'<a href="javascript:void(0)" class="open-post-metas-option">'+serverControlParams.i18n.mods.slider["the general options"]+"</a>","</strong>"].join(" ")+".";c.czr_Input("slide-title").container.prepend(b("<p/>",{html:d,"class":"czr-notice"}))},setModOptDependantsVisibilities:function(){var d=this,e=d.module,f=["slide-title","slide-subtitle","slide-cta","slide-link","slide-custom-link","slide-link-target"],g=e.czr_ModOpt();if(c.each(f,function(a){if(d.czr_Input.has(a)){var b=d.czr_Input(a);b.enabled(!e._isChecked(g["fixed-content"]))}}),e._isChecked(g["fixed-content"])){a.CZR_Helpers.setupDOMListeners([{trigger:"click keydown",selector:".open-mod-option",name:"toggle_mod_option",actions:function(){a.czr_ModOptVisible(!0,{module:e,focus:"section-topline-2"})}}],{model:d(),dom_el:d.container},d);var h=["<strong>",serverControlParams.i18n.mods.slider["The caption content is currently fixed and set in"],'<a href="javascript:void(0)" class="open-mod-option">'+serverControlParams.i18n.mods.slider["the general options"]+"</a>","</strong>"].join(" ")+".";d.czr_Input("slide-title").container.prepend(b("<p/>",{html:h,"class":"czr-fixed-content-notice"}))}else{var i=d.container.find(".czr-fixed-content-notice");!1!==i.length&&i.remove()}},toggleDisabledNotice:function(a){a=c.extend({before:"slide-title"},a)},setInputVisibilityDeps:function(){var a=this,b=a.module;a.czr_Input.each(function(d){switch(d.id){case"slide-link-title":a.czr_Input("slide-link").visible(b._isChecked(d())||!c.isEmpty(a.czr_Input("slide-cta")())),a.czr_Input("slide-link-target").visible(b._isChecked(d())||!c.isEmpty(a.czr_Input("slide-cta")())),d.bind(function(d){a.czr_Input("slide-link").visible(b._isChecked(d)||!c.isEmpty(a.czr_Input("slide-cta")())),a.czr_Input("slide-link-target").visible(b._isChecked(d)||!c.isEmpty(a.czr_Input("slide-cta")()))});break;case"slide-cta":a.czr_Input("slide-link").visible(!c.isEmpty(d())||b._isChecked(a.czr_Input("slide-link-title")())),a.czr_Input("slide-custom-link").visible(!c.isEmpty(d())&&b._isCustomLink(a.czr_Input("slide-link")())),a.czr_Input("slide-link-target").visible(!c.isEmpty(d())||b._isChecked(a.czr_Input("slide-link-title")())),d.bind(function(d){a.czr_Input("slide-link").visible(!c.isEmpty(d)||b._isChecked(a.czr_Input("slide-link-title")())),a.czr_Input("slide-custom-link").visible(!c.isEmpty(d)&&b._isCustomLink(a.czr_Input("slide-link")())),a.czr_Input("slide-link-target").visible(!c.isEmpty(d)||b._isChecked(a.czr_Input("slide-link-title")()))});break;case"slide-link":a.czr_Input("slide-custom-link").visible(b._isCustomLink(d())),d.bind(function(c){a.czr_Input("slide-custom-link").visible(b._isCustomLink(c))})}})},writeItemViewTitle:function(d,e){var f,g,h=this,i=1,j=h.module,k=d||h(),l="not_set",m=!c.isUndefined(e)&&c.isObject(e);if(!m||!e.input_changed||c.contains(["slide-title","slide-background"],e.input_changed)){if(c.isEmpty(k.title)){var n=c.findIndex(j.itemCollection(),function(a){return a.id===h.id});n=c.isUndefined(n)?i:n+1,f=[serverControlParams.i18n.mods.slider.Slide,n].join(" ")}else f=k.title;f=c.isEmpty(k["slide-title"])?f:k["slide-title"],f=a.CZR_Helpers.truncate(f,15),g=k["slide-background"]&&c.isString(k["slide-background"])?parseInt(k["slide-background"],10):k["slide-background"];var o=function(){return b.Deferred(function(){var a=this;serverControlParams.slideModuleParams&&serverControlParams.slideModuleParams.defaultThumb&&(l=serverControlParams.slideModuleParams.defaultThumb),c.isNumber(g)?wp.media.attachment(g).fetch().always(function(){var b=this;c.isObject(b)&&c.has(b,"attributes")&&c.has(b.attributes,"sizes")&&(l=this.get("sizes").thumbnail.url,a.resolve(l))}):a.resolve(l)}).promise()},p=b("."+j.control.css_attr.item_title,h.container).find(".slide-title"),q=b("."+j.control.css_attr.item_title,h.container).find(".slide-thumb");p.length?p.html(f):(b("."+j.control.css_attr.item_title,h.container).html(""),b("."+j.control.css_attr.item_title,h.container).append(b("<div/>",{"class":"slide-title",html:f})));var r=m&&e.input_changed&&"slide-background"===e.input_changed;0===q.length?o().done(function(a){"not_set"!=a&&b("."+j.control.css_attr.item_title,h.container).prepend(b("<div/>",{"class":"slide-thumb",html:'<img src="'+a+'" width="32" height="32" alt="'+f+'" />'}))}):!r&&m||o().done(function(a){"not_set"!=a&&q.html('<img src="'+a+'" width="32" height="32" alt="'+f+'" />')})}}}})}(wp.customize,jQuery,_);var CZRSlideModuleMths=CZRSlideModuleMths||{};!function(a,b,c){b.extend(CZRSlideModuleMths,{CZRSliderModOptCtor:{ready:function(){var b=this;b.module;b.inputCollection.bind(function(d){if(!c.isEmpty(d)){try{b.setModOptInputVisibilityDeps()}catch(e){a.errorLog("setModOptInputVisibilityDeps : "+e)}b.container.find(".refresh-button").prop("disabled",!0),a.CZR_Helpers.setupDOMListeners([{trigger:"click keydown",selector:".refresh-button",actions:function(d){a.previewer.refresh().done(function(){c.delay(function(){b.container.find(".refresh-button").prop("disabled",!0)},250)})}}],{model:b(),dom_el:b.container},b)}}),a.CZRModOpt.prototype.ready.call(b)},setModOptInputVisibilityDeps:function(){var a=this,b=a.module,d=function(){return b._isChecked(a.czr_Input("fixed-content")())};a.czr_Input.each(function(e){switch(e.id){case"fixed-content":var f=["fixed-title","fixed-subtitle","fixed-cta","fixed-link","fixed-link-target","fixed-custom-link","title-max-length","subtitle-max-length"],g=function(d,e){var f;switch(d){case"fixed-title":case"fixed-subtitle":case"fixed-cta":f=b._isChecked(e);break;case"fixed-link":case"fixed-link-target":f=b._isChecked(e)&&!c.isEmpty(a.czr_Input("fixed-cta")());break;case"fixed-custom-link":f=b._isChecked(e)&&!c.isEmpty(a.czr_Input("fixed-cta")())&&b._isCustomLink(a.czr_Input("fixed-link")());break;case"title-max-length":case"subtitle-max-length":f=!b._isChecked(e)}a.czr_Input(d).visible(f)};c.each(f,function(a){g(a,e())}),e.bind(function(a){c.each(f,function(b){g(b,a)})});break;case"fixed-cta":a.czr_Input("fixed-link").visible(!c.isEmpty(e())&&d()),a.czr_Input("fixed-custom-link").visible(!c.isEmpty(e())&&b._isCustomLink(a.czr_Input("fixed-link")())&&d()),a.czr_Input("fixed-link-target").visible(!c.isEmpty(e())&&d()),e.bind(function(e){a.czr_Input("fixed-link").visible(!c.isEmpty(e)&&d()),a.czr_Input("fixed-custom-link").visible(!c.isEmpty(e)&&b._isCustomLink(a.czr_Input("fixed-link")())&&d()),a.czr_Input("fixed-link-target").visible(!c.isEmpty(e)&&d())});break;case"fixed-link":a.czr_Input("fixed-custom-link").visible(b._isCustomLink(e())&&d()),e.bind(function(c){a.czr_Input("fixed-custom-link").visible(b._isCustomLink(c)&&d())});break;case"autoplay":a.czr_Input("slider-speed").visible(b._isChecked(e())),a.czr_Input("pause-on-hover").visible(b._isChecked(e())),e.bind(function(c){a.czr_Input("slider-speed").visible(b._isChecked(c)),a.czr_Input("pause-on-hover").visible(b._isChecked(c))});break;case"parallax":a.czr_Input("parallax-speed").visible(b._isChecked(e())),e.bind(function(c){a.czr_Input("parallax-speed").visible(b._isChecked(c))});break;case"post-metas":var h=["display-cats","display-comments","display-auth-date"],i=function(c,d){a.czr_Input(c).visible(b._isChecked(d))};c.each(h,function(a){i(a,e())}),e.bind(function(a){c.each(h,function(b){i(b,a)})})}})}}})}(wp.customize,jQuery,_);var CZRTextEditorModuleMths=CZRTextEditorModuleMths||{};!function(a,b,c){b.extend(CZRTextEditorModuleMths,{initialize:function(c,d){var e=this;a.CZRModule.prototype.initialize.call(e,c,d),b.extend(e,{itemInputList:"czr-module-text_editor-item-content"}),e.inputConstructor=a.CZRInput.extend(e.CZRTextEditorInputMths||{}),e.itemConstructor=a.CZRItem.extend(e.CZRTextEditorItem||{}),this.defaultItemModel={id:"",text:""}},CZRTextEditorInputMths:{},CZRTextEditorItem:{}})}(wp.customize,jQuery,_);var CZRSektionMths=CZRSektionMths||{};!function(a,b,c){b.extend(CZRSektionMths,{initialize:function(d,e){var f=this;a.CZRDynModule.prototype.initialize.call(f,d,e),b.extend(f,{itemPreAddEl:"czr-module-sektion-pre-add-view-content",rudItemPart:"czr-module-sektion-rud-item-part",itemInputList:"czr-module-sektion-view-content"}),f.defaultItemModel={id:"","sektion-layout":1,columns:[]},f.bind("pre_item_dom_remove",function(a){f.removeSektion(a)}),f.defaultDBColumnModel={id:"",sektion_id:"",modules:[]},f.defaultAPIcolumnModel={id:"",modules:[],sektion:{},module_id:"",control_id:"",is_added_by_user:!1},f.czr_Column=new a.Values,f.czr_columnCollection=new a.Value,f.czr_columnCollection.set([]),f.czr_columnCollection.callbacks.add(function(){return f.columnCollectionReact.apply(f,arguments)}),f.itemConstructor=a.CZRItem.extend(f.CZRSektionItem||{}),c.has(f,"modsDragInstance")||f.initModulesDragula(),a.czrModulePanelState=a.czrModulePanelState||new a.Value((!1)),a.czrModulePanelEmbedded=a.czrModulePanelEmbedded||b.Deferred(),f.userEventMap.set(c.union(f.userEventMap(),[{trigger:"click keydown",selector:".add-new-module",name:"add_new_module",actions:"toggleModuleListPanel"},{trigger:"click keydown",selector:"."+f.control.css_attr.open_pre_add_btn,name:"close_module_panel",actions:function(){a.czrModulePanelState(!1)}}])),a.consoleLog("SEKTION MODULE INIT",f.control.params.czr_skope),c.has(a,"czr_activeSkopeId")&&a.consoleLog("SEKTION MODULE INIT",a.czr_activeSkopeId()),a.czrModulePanelBinded=a.czrModulePanelBinded||b.Deferred(),"pending"==a.czrModulePanelBinded.state()&&(a.czrModulePanelState.bind(function(d){var e=a.CZR_Helpers.build_setId(f.control.params.syncCollection),g=a.control(e).syncSektionModule();if(b("body").toggleClass("czr-adding-module",d),d)g.renderModulePanel(),a.consoleLog("REACT TO MODULE PANEL STATE",d,f.control.params.syncCollection,g()),a.consoleLog("WHEN DOES THIS ACTION OCCUR?",a.czrModulePanelBinded.state()),g.modsDragInstance.containers.push(b("#czr-available-modules-list")[0]);else{var h=b.extend(!0,[],g.modsDragInstance.containers);h=c.filter(h,function(a){return"czr-available-modules-list"!=b(a).attr("id")}),g.modsDragInstance.containers=h,b("#czr-module-list-panel").remove()}}),a.czrModulePanelBinded.resolve()),a.czrSekSettingsPanelState=a.SekSettingsPanelState||new a.Value((!1)),a.czrSekSettingsPanelEmbedded=a.SekSettingsPanelEmbedded||b.Deferred(),f.userEventMap.set(c.union(f.userEventMap(),[{trigger:"click keydown",selector:".czr-edit-sek-settings",name:"edit_sek_settings",actions:"toggleSekSettingsPanel"},{trigger:"click keydown",selector:"."+f.control.css_attr.open_pre_add_btn,name:"close_sektion_panel",actions:function(){a.czrSekSettingsPanelState.set(!1)}}])),a.czrSekSettingsPanelEmbedded.done(function(){a.czrSekSettingsPanelState.callbacks.add(function(){return f.reactToSekSettingPanelState.apply(f,arguments)})}),a.section(f.control.section()).expanded.bind(function(b){a.consoleLog("FIRE SEKTION MODULE!",f.id),f.fireSektionModule()})},fireSektionModule:function(){var a=this;"resolved"!=a.isReady.state()&&(a.ready(),a.control.getSyncCollectionControl().syncSektionModule.set(a))},removeSektion:function(a){var b=this;c.each(a.columns,function(a){c.each(a.modules,function(a){b.control.getSyncCollectionControl().removeModule(a)}),b.czr_Column.has(a.id)&&"resolved"==b.czr_Column(a.id).embedded.state()&&b.czr_Column(a.id).container.remove(),b.removeColumnFromCollection(a)})},closeAllOtherSektions:function(a){var b=this;_clicked_sektion_id=a.closest(".czr-single-item").attr("data-id"),b.czr_Item.each(function(a){_clicked_sektion_id!=a.id?a.viewState.set("closed"):a.viewState.set("expanded"!=a.viewState()?"expanded_noscroll":"expanded")})}})}(wp.customize,jQuery,_);var CZRSektionMths=CZRSektionMths||{};!function(a,b,c){b.extend(CZRSektionMths,{CZRSektionItem:{initialize:function(d,e){var f=this;a.CZRItem.prototype.initialize.call(f,null,e),f.userEventMap.set(c.union(f.userEventMap(),[{trigger:"click keydown",selector:["."+f.module.control.css_attr.edit_view_btn,"."+f.module.control.css_attr.display_alert_btn,"."+f.module.control.css_attr.item_title].join(","),name:"close_module_panel",actions:function(){a.czrModulePanelState.set(!1)}},{trigger:"mouseenter",selector:".czr-item-header",name:"hovering_sek",actions:function(a){f.module.control.previewer.send("start_hovering_sek",{id:f.id})}},{trigger:"mouseleave",selector:".czr-item-header",name:"hovering_sek",actions:function(a){f.module.control.previewer.send("stop_hovering_sek",{id:f.id})}},{trigger:"click keydown",selector:["."+f.module.control.css_attr.edit_view_btn,"."+f.module.control.css_attr.item_title].join(","),name:"send_edit_view",actions:function(a){f.module.control.previewer.send("edit_sek",{id:f.id})}}]));var g=f(),h=e.module;if(!c.has(g,"sektion-layout"))throw new Error("In Sektion Item initialize, no layout provided for "+f.id+".");f.isReady.done(function(){if(c.isEmpty(f().columns)){var a=parseInt(g["sektion-layout"]||1,10);for(i=1;i<a+1;i++){var d=b.extend(!0,{},h.defaultDBColumnModel),e={id:"",sektion_id:f.id};e=b.extend(d,e),h.instantiateColumn(b.extend(e,{sektion:f}))}}else c.each(f().columns,function(a){var c=b.extend(!0,{},a);h.instantiateColumn(b.extend(c,{sektion:f}))})})},itemReact:function(c,d){var e=this,f=b.extend(!0,{},c);f=e.prepareSekItemForDB(f),a.CZRItem.prototype.itemReact.call(e,f)},prepareSekItemForDB:function(a){var b=this,d={};return c.each(b.module.defaultItemModel,function(e,f){var g=a[f];switch(f){case"id":if(!c.isString(g)||c.isEmpty(g))throw new Error("The sekItem id property must be a not empty string");d[f]=g;break;case"sektion-layout":if(!c.isNumber(parseInt(g,10))||parseInt(g,10)<1)throw new Error("The sekItem layout property must be an int number > 0");d[f]=g;break;case"columns":if(!c.isArray(g))throw new Error("The sekItem columns property must be an array");var h=[];c.each(g,function(a){var c=b.module.prepareColumnForDB(a);h.push(c)}),d[f]=h}}),d}}})}(wp.customize,jQuery,_);var CZRSektionMths=CZRSektionMths||{};!function(a,b,c){b.extend(CZRSektionMths,{prepareColumnForDB:function(a){var b=this,d={};return c.each(b.defaultDBColumnModel,function(b,e){var f=a[e];switch(e){case"id":if(!c.isString(f)||c.isEmpty(f))throw new Error("The column id property must be a not empty string");d[e]=f;break;case"sektion_id":if(c.isString(f)&&!c.isEmpty(f))d[e]=f;else{if(!c.has(a,"sektion"))throw new Error("The column sektion-id property must be a not empty string");d[e]=a.sektion.id}break;case"modules":if(!c.isArray(f))throw new Error("The column modules property must be an array");d[e]=f}}),d},instantiateColumn:function(b,d){var e=this,f=c.clone(b);if(!c.isEmpty(f.id)&&e.czr_Column.has(f.id))throw new Error("The column id already exists in the collection in module : "+e.id);f=e.prepareColumnForAPI(f),e.czr_Column.add(f.id,new a.CZRColumn(f.id,f)),e.czr_Column(f.id).ready()},prepareColumnForAPI:function(a){var b=this,d={};if(!c.isObject(a))throw new Error("Sektion Module::prepareColumnForAPI : a column must be an object to be instantiated.");return c.each(b.defaultAPIcolumnModel,function(e,f){var g=a[f];switch(f){case"id":c.isEmpty(g)?d[f]=b.generateColId():d[f]=g;break;case"modules":if(!c.isArray(g))throw new Error("Sektion Module::prepareColumnForAPI : a collection of modules must be an array. Error in column "+a.id);d[f]=g;break;case"sektion":if(!c.isObject(g)||c.isEmpty(g))throw new Error("Sektion Module::prepareColumnForAPI : a sektion instance is missing for column "+a.id);d[f]=g;break;case"module_id":d[f]=b.id;break;case"control_id":d[f]=b.control.id;break;case"is_added_by_user":d[f]=!!c.isBoolean(g)&&g}}),d},updateColumnCollection:function(d){var e=this,f=e.czr_columnCollection();if(_new_collection=b.extend(!0,[],f),a.consoleLog("in update column collection",e.id,e.czr_columnCollection()),c.has(d,"collection"))return void e.czr_columnCollection.set(d.collection);if(!c.has(d,"column"))throw new Error("updateColumnCollection, no column provided in module "+e.id+". Aborting");var g=c.clone(d.column);if(!c.has(g,"id"))throw new Error("updateColumnCollection, no id provided for a column in module"+e.id+". Aborting");c.findWhere(_new_collection,{id:g.id})?c.each(f,function(a,b){a.id==g.id&&(_new_collection[b]=g)}):_new_collection.push(g),e.czr_columnCollection.set(_new_collection)},removeColumnFromCollection:function(a){var d=this,e=d.czr_columnCollection(),f=b.extend(!0,[],e);f=c.filter(f,function(b){return b.id!=a.id}),d.czr_columnCollection.set(f)},columnCollectionReact:function(a,d){var e=this,f=c.size(d)<c.size(a),g=c.size(d)>c.size(a),h=c.size(d)==c.size(a),i={},j={};if(h&&c.each(a,function(a,e){c.isEqual(a,d[e])||(i=a.sektion(),j=b.extend(!0,{},i),c.each(i.columns,function(b,c){b.id==a.id&&(j.columns[c]=a)}),a.sektion.set(j))}),f){var k=c.filter(a,function(a){return c.isUndefined(c.findWhere(d,{id:a.id}))});k=k[0],i=k.sektion(),c.isUndefined(c.findWhere(i.columns,{id:k.id}))&&(j=b.extend(!0,{},i),j.columns.push(k),k.sektion.set(j))}if(g){var l=c.filter(d,function(b){return c.isUndefined(c.findWhere(a,{id:b.id}))});l=l[0],i=l.sektion(),j=b.extend(!0,{},i),j.columns=c.filter(j.columns,function(a){return a.id!=l.id}),l.sektion.set(j),e.czr_Column.remove(l.id)}},generateColId:function(a,b){if(b=b||1,b>100)throw new Error("Infinite loop when generating of a column id.");var d=this;a=a||d._getNextColKeyInCollection();var e="col_"+a;if(!c.has(d,"czr_columnCollection")||!c.isArray(d.czr_columnCollection()))throw new Error("The column collection does not exist or is not properly set in module : "+d.id);return d.czr_Column.has(e)?d.generateColId(a++,b++):e},_getNextColKeyInCollection:function(){var a=this,b={},d=0;return c.isEmpty(a.czr_columnCollection())||(b=c.max(a.czr_columnCollection(),function(a){return parseInt(a.id.replace(/[^\/\d]/g,""),10)}),d=parseInt(b.id.replace(/[^\/\d]/g,""),10)+1),d},moduleExistsInOneColumnMax:function(a){return 2>this.getModuleColumn(a).length},getModuleColumn:function(a){var b=this,d=[];return c.each(b.czr_columnCollection(),function(b,e){c.findWhere(b.modules,{id:a})&&d.push(b.id)}),d}})}(wp.customize,jQuery,_);var CZRSektionMths=CZRSektionMths||{};!function(a,b,c){b.extend(CZRSektionMths,{initModulesDragula:function(){var a=this;a.modsDragInstance=dragula({copy:function(a,c){return b(a).hasClass("czr-module-candidate")},moves:function(a,b,d,e){return c.contains(d.className.split(" "),"czr-mod-drag-handler")},accepts:function(a,d,e,f){return!c.isUndefined(d)&&"czr-available-modules-list"!=b(d).attr("id")},isContainer:function(a){return!1}}),a.modsDragInstance.on("drag",function(b,c){a.czr_Item.each(function(a){a.viewState.set("expanded"!=a.viewState()?"expanded_noscroll":"expanded")})}).on("dragend",function(a,b){}).on("drop",function(d,e,f,g){var h=b(d).attr("data-module-id"),i=b(d).attr("data-module-type"),j=b(e).closest(".czr-column").attr("data-id"),k=b(f).closest(".czr-column").attr("data-id"),l=j==k,m=b(d).hasClass("czr-module-candidate");if(m){if(c.isUndefined(j)||c.isUndefined(i))return;a.userAddedModule(j,i),a.reorderModulesInColumn(j)}else l?a.reorderModulesInColumn(j):a.control.getSyncCollectionControl().czr_Module(h).modColumn.set(j)});autoScroller([a.control.container.closest(".accordion-section-content")[0]],{direction:"vertical",margin:20,pixels:100,scrollWhenOutside:!0,autoScroll:function(){return a.modsDragInstance.dragging}})},userAddedModule:function(a,d){var e=this,f=e.control.getSyncCollectionControl(),g=c.clone(f.getDefaultModuleApiModel());f.trigger("user-module-candidate",b.extend(g,{module_type:d,column_id:a,sektion:e.czr_Column(a).sektion,sektion_id:e.czr_Column(a).sektion.id,is_added_by_user:!0}))},reorderModulesInColumn:function(a){var b=this,c=b.czr_Column(a).getColumnModuleCollectionFromDom(a);b.czr_Column(a).updateColumnModuleCollection({collection:c})},moveModuleFromTo:function(b,d,e){a.consoleLog("ALORS CE BUG?",this(),this.czr_columnCollection());var f=this,g=f.czr_Column(e).getColumnModuleCollectionFromDom(d);c.has(a,"czrModulePanelState")&&a.czrModulePanelState(!1),f.czr_Column(e).updateColumnModuleCollection({collection:g}),f.czr_Column(d).removeModuleFromColumnCollection(b)}})}(wp.customize,jQuery,_);var CZRSektionMths=CZRSektionMths||{};!function(a,b,c){b.extend(CZRSektionMths,{toggleModuleListPanel:function(c){var d=this;a.czrSekSettingsPanelState.set(!1),a.czrModulePanelState.set(!a.czrModulePanelState()),a.czrModulePanelState()?d.czr_Item.each(function(a){a.viewState.set("expanded"!=a.viewState()?"expanded_noscroll":"expanded")}):d.closeAllOtherSektions(b(c.dom_event.currentTarget,c.dom_el))},renderModulePanel:function(){if(0===b("#tmpl-czr-available-modules").length)throw new Error("No template found to render the module panel list");b("#widgets-left").after(b(wp.template("czr-available-modules")())),c.each(a.czrModuleMap,function(a,c){var d=b("<li/>",{"class":"czr-module-candidate","data-module-type":c,html:'<h3><span class="czr-mod-drag-handler fa fa-arrows-alt"></span>'+a.name+"</h3>"});b("#czr-available-modules-list").append(d)})}})}(wp.customize,jQuery,_);var CZRColumnMths=CZRColumnMths||{};!function(a,b,c){b.extend(CZRColumnMths,{initialize:function(c,d){var e=this;a.Value.prototype.initialize.call(e,null,d),b.extend(e,d||{}),e.isReady=b.Deferred(),e.embedded=b.Deferred(),e.czr_columnModuleCollection=new a.Value,e.czr_columnModuleCollection.set(e.modules),e.set(d),e.defautModuleModelInColumn={id:""},e.sektion.bind("contentRendered",function(){e.container=e.render(),a.consoleLog("COLUMN CONTAINER?",e.container),e.embedded.resolve()}),e.embedded.done(function(){e.mayBeInstantiateColumnModules(),e.callbacks.add(function(){return e.columnReact.apply(e,arguments)}),e.czr_columnModuleCollection.callbacks.add(function(){return e.columnModuleCollectionReact.apply(e,arguments);
}),a.CZR_Helpers.setupDOMListeners(e.column_event_map,{dom_el:e.container},e);var c=a.control(e.control_id).getSyncCollectionControl();a.consoleLog("////////////////////////////////////////////////////"),a.consoleLog("column.container?",e.container),a.consoleLog("syncCollectionControl.syncSektionModule()",c.syncSektionModule()()),a.consoleLog("////////////////////////////////////////////////////"),c.syncSektionModule().modsDragInstance.containers.push(b(".czr-module-collection-wrapper",e.container)[0])})},ready:function(){var a=this;a.isReady.resolve(),a.sektion.module.updateColumnCollection({column:a()})},mayBeInstantiateColumnModules:function(){var d=this,e=d.sektion.control.getSyncCollectionControl();b.when(e.moduleCollectionReady.promise()).then(function(){c.each(d.czr_columnModuleCollection(),function(a){e.czr_Module.has(a.id)||b.when(c.findWhere(e.czr_moduleCollection(),{id:a.id})).done(function(b){if(c.isUndefined(b)||c.isEmpty(b))throw new Error("Module "+a.id+" was not found in the module collection.");e.instantiateModule(b,{}).ready()})})},function(){},function(){a.consoleLog("NOT SYNCHRONIZED YET")})},render:function(){var a=this;return $view=b(wp.template("czr-sektion-column")({id:a.id})),$view.appendTo(b(".czr-column-wrapper",a.sektion.container)),$view},columnReact:function(a,b){this.sektion.module.updateColumnCollection({column:a})}})}(wp.customize,jQuery,_);var CZRColumnMths=CZRColumnMths||{};!function(a,b,c){b.extend(CZRColumnMths,{updateColumnModuleCollection:function(d){var e=this,f=e.czr_columnModuleCollection();if(_new_collection=b.extend(!0,[],f),a.consoleLog("column.czr_columnModuleCollection()",e.czr_columnModuleCollection()),c.has(d,"collection"))return void e.czr_columnModuleCollection.set(d.collection);if(!c.has(d,"module"))throw new Error("updateColumnModuleCollection, no module provided in column "+e.id+". Aborting");var g=e.prepareModuleForColumnAPI(c.clone(d.module));c.findWhere(_new_collection,{id:g.id})?c.each(f,function(a,b){a.id==g.id&&(_new_collection[b]=g)}):_new_collection.push(g),e.czr_columnModuleCollection.set(_new_collection)},columnModuleCollectionReact:function(a,b){var d=this,e=d(),f=c.clone(e),g=[];c.each(a,function(a,b){g[b]={id:a.id}}),f.modules=g,d.set(f)},removeModuleFromColumnCollection:function(a){var d=this,e=d.czr_columnModuleCollection();_new_collection=b.extend(!0,[],e),_new_collection=c.filter(_new_collection,function(b){return b.id!=a.id}),d.czr_columnModuleCollection.set(_new_collection)},prepareModuleForColumnAPI:function(a){if(!c.isObject(a))throw new Error("prepareModuleForColumnAPI : a module must be an object.");var b=this,d={};return c.each(b.defautModuleModelInColumn,function(e,f){var g=a[f];switch(f){case"id":if(!c.isString(g)||c.isEmpty(g))throw new Error("prepareModuleForColumnAPI : a module id must a string not empty");if(!b.sektion.module.moduleExistsInOneColumnMax(a.id))throw new Error("A module can not be embedded in more than one column at a time. Module "+a.id+" exists in several columns : "+b.sektion.module.getModuleColumn(a.id).join(","));d[f]=g}}),d},getColumnModuleCollectionFromDom:function(d){var e=this,f=b(".czr-module-collection-wrapper",e.container),g=e.sektion.module.czr_Column(d).czr_columnModuleCollection(),h=[];if(a.consoleLog("in GET COLUMN MODULE COLLECTION FROM DOM",d,f,e.container),b(".czr-single-module",f).each(function(a){if(!c.isUndefined(c.findWhere(e.czr_columnModuleCollection(),{id:b(this).attr("data-module-id")})))return void(h[a]=c.findWhere(e.czr_columnModuleCollection(),{id:b(this).attr("data-module-id")}));var f=c.findWhere(g,{id:b(this).attr("data-module-id")});if(!f)throw new Error("The module  : "+b(this).attr("data-module-id")+" was not found in the collection of its previous column "+d);h[a]=e.prepareModuleForColumnAPI(f)}),c.isEmpty(h))throw new Error("There was a problem when re-building the column module collection from the DOM in column : "+e.id);return h}})}(wp.customize,jQuery,_);var CZRSektionMths=CZRSektionMths||{};!function(a,b,c){b.extend(CZRSektionMths,{toggleSekSettingsPanel:function(c){var d=this;if("pending"==a.czrSekSettingsPanelEmbedded.state())try{b.when(d.renderSekSettingsPanel()).done(function(){a.czrSekSettingsPanelEmbedded.resolve()})}catch(e){a.errorLog("In toggleSekSettingsPanel : "+e)}a.czrModulePanelState.set(!1),a.czrSekSettingsPanelState.set(!a.czrSekSettingsPanelState()),d.closeAllOtherSektions(b(c.dom_event.currentTarget,c.dom_el))},reactToSekSettingPanelState:function(a){b("body").toggleClass("czr-editing-sektion",a)},renderSekSettingsPanel:function(){var c="";if(0===b("#tmpl-czr-sektion-settings-panel").length)throw new Error("No template found to render the sektion setting panel");try{c=wp.template("czr-sektion-settings-panel")()}catch(d){return void a.errorLog("Error when parsing the template of the sektion setting panel"+d)}b("#widgets-left").after(b(c))}})}(wp.customize,jQuery,_),function(a,b,c){a.czrModuleMap=a.czrModuleMap||{},b.extend(a.czrModuleMap,{czr_sektion_module:{mthds:CZRSektionMths,crud:!0,name:"Sektions"},czr_fp_module:{mthds:CZRFeaturedPageModuleMths,crud:!0,name:"Featured Pages"},czr_slide_module:{mthds:CZRSlideModuleMths,crud:!0,name:"Slider",has_mod_opt:!0},czr_text_module:{mthds:CZRTextModuleMths,crud:!1,multi_item:!1,name:"Simple Text"},czr_text_editor_module:{mthds:CZRTextEditorModuleMths,crud:!1,multi_item:!1,name:"WP Text Editor"}})}(wp.customize,jQuery,_);