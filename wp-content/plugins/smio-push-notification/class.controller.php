...
class smpush_controller extends smpush_helper{
  ...
  public static $data;
  public static $platforms = array('ios','android','wp','wp10','bb','chrome','safari','firefox');
  public static $platform_titles = array('iOS','Android','Windows Phone','Windows 10','BlackBerry','Chrome','Safari','Firefox');
    ...
    if($index === false){
      if(isset(self::$data[$field])){
        if(is_array(self::$data[$field])){
          return array_map('stripslashes', self::$data[$field]);
        }
        else{
      ...
      if(isset(self::$data[$field][$index])){
        if(is_array(self::$data[$field][$index])){
          return array_map('stripslashes', self::$data[$field][$index]);
        }
        else{
  ...
  public function build_menus(){
    ...
    add_submenu_page('smpush_setting', __('Campaigns List', 'smpush-plugin-lang'), __('Campaigns List', 'smpush-plugin-lang'), 'delete_pages', 'smpush_archive', array('smpush_modules', 'archive'));
  ...
  public function cron_setup(){
    if(!wp_next_scheduled('smpush_silent_cron')){
      wp_schedule_event(mktime(date('H'),0,0,date('m'),date('d'),date('Y')), 'hourly', 'smpush_silent_cron');
	}
  ...
  public static function update_counters(){
    ...
    $wpdb->query("DELETE FROM ".$wpdb->prefix."push_desktop_messages WHERE timepost<NOW()-INTERVAL 30 DAY");
  ...
  public static function run_silent_cron(){
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, get_bloginfo('url').'/index.php?smpushcontrol=cron_job');
    curl_setopt($ch, CURLOPT_HEADER, 0);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
