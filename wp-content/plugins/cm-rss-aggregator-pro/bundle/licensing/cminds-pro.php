...
if ( !defined( __NAMESPACE__ . '\PLATFORM_VERSION' ) ) {
	define( __NAMESPACE__ . '\PLATFORM_VERSION', '1_6_2' );
}
	...
	class CmindsProPackage {
		...
		public function getUserguideUrl() {
			...
			if ( $this->getOption( 'plugin-userguide-key' ) ) {
				...
				$url .= 'https://www.cminds.com/wordpress-plugins-support-documentation/';
			}
			return $url;
		}
...
//						'30' => 'Upgrade Tutorial',
		...
		public function showAds( $atts = array() ) {
			 ...
			 */
			$adsHidden		 = get_option( $optionName, true );
...
// Handle current for post_type=post|page|foo pages, which won't match $self.
		...
		public function cminds_compatibility_check( $plaintext = false ) {
			...
			$mod_rewrite = (FALSE !== strpos( $phpinfo, 'mod_rewrite' ));
			...
			if ( !$plaintext ):
				...
				<table class="form-table server-info-table">
					...
					<tr>
						...
						<td><?php echo ($mod_rewrite) ? 'Yes' : 'No' ?></td>
						<td><?php if ( empty( $mod_rewrite ) ): ?><strong>"mod_rewrite" module is required for permalinks to work.</strong><?php else: ?><span>OK</span><?php endif; ?></td>
					</tr>
					<tr>
						<td>PHP Version</td>
