...
class Mailchimp extends Action_Base {
	...
	public function run( $record, $ajax_handler ) {
		...
		$subscriber['status'] = 'yes' === $form_settings['mailchimp_double_opt_in'] ? 'pending' : 'subscribed';

		$response = $handler->post( 'lists/' . $form_settings['mailchimp_list'] . '/members/', $subscriber );

		if ( 200 !== $response['code'] && 'Member Exists' !== $response['body']['title'] ) {
